  ...
  create(context) {
    return {
      ImportDeclaration(node) {
        ...
        if (context.getFilename().match(protocolFileMatch) && isMetamodelImport) {
          node.specifiers.forEach(specifier => {
            ...
            if (!specifier.local.name.startsWith('MM_')) {
              context.report({
                node: specifier,
                message: 'Prefer aliasing with prefix \'MM_\' for metamodel imports in version-specific protocol code',
              });
            }
          });
        }
