...
const protocolModelFileMatch = /[/\\]app[/\\]models[/\\]protocols[/\\]pure[/\\]v(?<protocolVersion>.*?)[/\\]model[/\\]/u;
  ...
  create(context) {
    return {
      ImportDeclaration(node) {
        ...
        if (context.getFilename().match(protocolModelFileMatch) && isMetamodelImport) {
          context.report({
            node: node,
            message: 'Disallow dependency on metamodels in protocol model definitions, use visitor for processing instead',
          });
        }
      },
    };
