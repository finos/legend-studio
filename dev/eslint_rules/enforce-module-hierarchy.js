...
const utilityFileMatch = /[/\\]app[/\\]utils[/\\]/u;
const storeFileMatch = /[/\\]app[/\\]stores[/\\]/u;
const componentFileMatch = /[/\\]app[/\\]components[/\\]/u;
const entryFileMatch = /[/\\]app[/\\]index\.tsx$/u;
...
module.exports = {
  ...
  create(context) {
    ...
    function getImportModule(node) {
      ...
      if (val.match(/^V\d*[/\\]/u) || (val.startsWith('./') && path.resolve(context.getFilename(), val).match(protocolFileMatch))) {
        return PROTOCOL;
      } else if (val.startsWith('MM/') || (val.startsWith('./') && path.resolve(context.getFilename(), val).match(metamodelFileMatch))) {
        return METAMODEL;
      } else if (val.match('Utilities/') || (val.startsWith('./') && path.resolve(context.getFilename(), val).match(utilityFileMatch))) {
        return UTIL;
      } else if (val.match('Stores/') || (val.startsWith('./') && path.resolve(context.getFilename(), val).match(storeFileMatch))) {
        return STORE;
      } else if (val.match('Components/') || (val.startsWith('./') && path.resolve(context.getFilename(), val).match(componentFileMatch))) {
        return COMPONENT;
      }
      return OTHER;
    }
    ...
    function getCurrentFileModule() {
      if (path.resolve(context.getFilename(), '.').match(protocolFileMatch)) {
        return PROTOCOL;
      } else if (path.resolve(context.getFilename(), '.').match(metamodelFileMatch)) {
        return METAMODEL;
      } else if (path.resolve(context.getFilename(), '.').match(utilityFileMatch)) {
        return UTIL;
      } else if (path.resolve(context.getFilename(), '.').match(storeFileMatch)) {
        return STORE;
      } else if (path.resolve(context.getFilename(), '.').match(componentFileMatch) || path.resolve(context.getFilename(), '.').match(entryFileMatch)) {
        return COMPONENT;
      }
      return OTHER;
    }
    ...
    return {
      ImportDeclaration(node) {
        ...
        if (FORBIDDEN_DEPENDENCY[currentFileModule] && FORBIDDEN_DEPENDENCY[currentFileModule].includes(importModule)) {
          context.report({
            node: node,
