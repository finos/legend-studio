...
const isMatchingKeyCombination = (
  ...
  // NOTE: we allow specifying with either `event.key` or the `event.code`
  ...
  (combinationData.key === event.code ||
    // uppercase the key so it behaves somewhat similar to `event.code` (i.e. we don't have to take into account effect of Shift)
    combinationData.key.toUpperCase() === event.key.toUpperCase()) &&
  ...
  return (event) => {
    ...
    Object.values(config)
      ...
      .forEach((entry) => {
        ...
        if (!matches) {
          // if a modifier that is part of the current keyboard event
