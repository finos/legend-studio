 ...
 * metamodel vs. protocol imports, and it seems like ES6 module improts are hoisted (*needs citation)
 *
  ...
  create(context) {
    ...
    function validateImportChunk(importChunk, sourceCode) {
      ...
      importChunk.forEach((importDeclarationNode, idx) => {
        ...
        importGroupIndexMap.set(
          currentImportGroup,
          (importGroupIndexMap.get(currentImportGroup) || []).concat([idx]),
        );
        const currentImportGroupIndex = importGroups.indexOf(
          currentImportGroup,
        );
      ...
      if (!passedValidation) {
        importChunkString = importGroups
          .map((importGroup) => {
            if (!importGroupIndexMap.has(importGroup)) {
              return undefined;
            }
            return importGroupIndexMap
              .get(importGroup)
              .map((importIndex) =>
                sourceCode.getText(importChunk[importIndex]),
              )
              .join('\n');
          })
          .filter(Boolean)
    ...
    return {
      Program(programNode) {
        for (const importChunk of extractImportChunks(programNode)) {
          ...
          const [passedValidation, sortedImports] = validateImportChunk(
            importChunk,
            sourceCode,
          );
          ...
          if (!passedValidation) {
            context.report({
              ...
              fix: (fixer) =>
                fixer.replaceTextRange([start, end], sortedImports),
            });
          }
        }
      },
