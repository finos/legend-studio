...
const protocolCommonFileMatch = /[/\\]src[/\\]models[/\\]protocols[/\\]pure[/\\][^/\\]*$/u;
      ...
      ImportDeclaration(node) {
        ...
        if (importProtocolVersion) {
          if (!context.getFilename().match(protocolCommonFileMatch)) {
            const currentFileProtocolVersionMatch = context
              .getFilename()
              .match(protocolFileMatch);
            const currentFileProtocolVersion = currentFileProtocolVersionMatch
              ? currentFileProtocolVersionMatch.groups.protocolVersion
              : undefined;
            if (currentFileProtocolVersion !== importProtocolVersion) {
              context.report({
                node: node.source,
                message: `Protocol models V${importProtocolVersion} cannot be used ${
                  currentFileProtocolVersion
                    ? `in definition of protocol V${currentFileProtocolVersion}`
                    : "outside of 'src/models/protocols/pure/'"
                }`,
              });
            }
          }
