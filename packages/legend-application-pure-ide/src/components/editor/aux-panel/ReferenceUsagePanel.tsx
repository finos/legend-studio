...
const UsageResultDisplay = observer(
  (props: { referenceUsageState: ReferenceUsageResult }) => {
    ...
    const refresh = (): void => {
      flowResult(
        editorStore.findUsages(referenceUsageState.usageConcept),
      ).catch(applicationStore.alertUnhandledError);
    };
    const clear = (): void => editorStore.setReferenceUsageResult(undefined);
...
export const ReferenceUsagePanel = observer(() => {
  ...
  return (
    <div className="references-panel">
      <PanelLoadingIndicator
        isLoading={editorStore.referenceUsageLoadState.isInProgress}
      />
      {!editorStore.referenceUsageResult && (
        <BlankPanelContent>
      ...
      {editorStore.referenceUsageResult && (
        <UsageResultDisplay
          referenceUsageState={editorStore.referenceUsageResult}
        />
      )}
    </div>
  );
