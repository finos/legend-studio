...
export class EditorSplitGroupState implements EditorSplitNode {
  ...
  constructor(
    ...
    // Ensure child nodes have correct parent linkage
    if (
...
export class EditorSplitRootState {
  ...
  constructor(ideStore: PureIDEStore) {
    makeObservable(this, {
      ...
      closeTab: action,
      splitLeaf: action,
      splitActiveLeaf: action,
      moveTab: action,
      moveTabAtIndex: action,
      closeAllTabs: action,
      closeAllTabsInLeaf: action,
      removeSplit: action,
      unsplitAll: action,
      leaves: computed,
      currentTab: computed,
  ...
  splitLeaf(
    ...
    if (this.root === leaf) {
      ...
      const newGroup = new EditorSplitGroupState(orientation, leaf, newLeaf);
      this.setRoot(newGroup);
    } else if (leaf.parent) {
      // Replace the leaf in its original parent with a new group
      ...
      if (parent.first === leaf) {
        parent.setFirst(newGroup);
      } else {
        parent.setSecond(newGroup);
      }
    }
  ...
  moveTabAtIndex(
    ...
    if (sourceLeaf && sourceLeaf !== targetLeaf) {
      // Remove from source without triggering close hooks
      const tabIndex = sourceLeaf.tabManagerState.tabs.indexOf(tab);
      if (tabIndex !== -1) {
        sourceLeaf.tabManagerState.tabs.splice(tabIndex, 1);
        if (sourceLeaf.tabManagerState.currentTab === tab) {
          sourceLeaf.tabManagerState.setCurrentTab(
            sourceLeaf.tabManagerState.tabs[0],
          );
        }
      }

      // Insert into target at index respecting pinned semantics
      targetLeaf.tabManagerState.insertTabAtIndex(tab, index);
      this.setActiveLeaf(targetLeaf);
      ...
      // No-op for intra-leaf; reordering handled by TabManager
      targetLeaf.tabManagerState.setCurrentTab(tab);
      this.setActiveLeaf(targetLeaf);
    }
  }
  ...
  closeAllTabsInLeaf(leaf: EditorSplitLeaf): void {
    if (!this.leaves.includes(leaf)) {
      return;
    }
    const tabs = [...leaf.tabManagerState.tabs];
    tabs.forEach((tab) => leaf.tabManagerState.closeTab(tab));
    // Keep the split layout; do not auto-cleanup here so users can reuse the pane.
    // Maintain active leaf on the emptied pane to allow immediate interactions.
    this.setActiveLeaf(leaf);
  }
