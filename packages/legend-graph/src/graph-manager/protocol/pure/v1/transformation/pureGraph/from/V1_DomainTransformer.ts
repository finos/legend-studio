...
export const V1_createGenericType = (
  ...
  const protocolGenType = V1_createGenericTypeWithElementPath(genericType.rawType.path);
  const typeArguments = genericType.typeArguments ?? [];
  protocolGenType.typeArguments = typeArguments.map((t) => V1_createGenericType(t.value));
