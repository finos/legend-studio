...
test(unitTest('Generate default parameter value for type'), async () => {
  ...
  expect(
      getFunctionSignature(setFunction),
  ).toBe(
  ...
  expect(
      generateFunctionPrettyName(relationFunction, { fullPath: true, spacing: true }),
  ).toBe(
      'model::functions::relationFunction(): Relation<Any>[0..1]',
  );
  expect(
      getFunctionSignature(relationFunction),
  ).toBe(
      '__Relation_$0_1$_',
  );
