...
export const DataQualityRelationComparisonEditor = observer(() => {
  ...
  const keysInstance = useMemo(() => {
    const instance = new ColSpecArrayInstance(Multiplicity.ZERO_MANY);
    const arr = new ColSpecArray();
    arr.colSpecs = comparison.keys.map((col) => {
      const spec = new ColSpec();
      spec.name = col;
      return spec;
    });
    instance.values = [arr];
  ...
  const columnsToCompareInstance = useMemo(() => {
    const instance = new ColSpecArrayInstance(Multiplicity.ZERO_MANY);
    const arr = new ColSpecArray();
    arr.colSpecs = comparison.columnsToCompare.map((col) => {
      const spec = new ColSpec();
      spec.name = col;
      return spec;
    });
        ...
        <PanelContent>
          ...
          <div className="data-quality-relation-comparison-editor__panel__content__form">
            <PanelFormSection>
              ...
              <RenderColumns
                columns={keysInstance}
                onChange={(vals) => state.setKeys(vals)}
                options={combinedColumnOptions.map((o) => ({
                  ...o,
                  type: '',
                }))}
            ...
            <PanelFormSection>
              ...
              <RenderColumns
                columns={columnsToCompareInstance}
                onChange={(vals) => state.setColumnsToCompare(vals)}
                options={combinedColumnOptions.map((o) => ({
                  ...o,
                  type: '',
                }))}
