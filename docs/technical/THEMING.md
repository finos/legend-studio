...
- **QB dark-theme parity restoration (July 2026):** production feedback after #5299 shipped: Legend Query dark users reported "different shades everywhere." A line-paired audit against the pre-tokenization baseline (`3d6d4563d~1`) found **331 declarations whose resolved dark value changed** — the Phase 1 "mechanical" pass was not value-preserving (e.g. `dark-grey-100` backgrounds → `bg-panel-header` which resolves `dark-grey-200`), and Phase 2's canonical mappings changed muted-grey text (`dark-grey-300/400/500` → bright `light-grey-400`, up to Δ98), tree-connector borders (light `light-grey-0/300` → dark `border-strong/default`, Δ150+), error reds, and accent blues. Fixed 226 of them for exact dark parity: value-preserving token swaps where a token with the right dark value exists (`bg-panel-header`→`bg-panel` etc.), palette reverts for saturated stable colors and dark-grey text (safe in light theme). The ~100 kept differences are all single-notch (Δ≤19/255) light-grey text/border shifts retained **because reverting them would break the now-in-prod Studio light theme** (e.g. `light-grey-100` text reverted would be invisible on white); plus the whitelisted undefined-var bug fixes. Audit + fixer scripts: `dark_parity_audit.py` / `dark_parity_fix.py` (session scratchpad; re-derivable — the method is: difflib line-pairing old↔new SCSS, resolve semantic→dark palette, compare, then policy-based replacement). **Lessons:** (1) mechanical tokenization passes must be _value-preserving_ by construction — map each palette value to the token that resolves to it, never to the semantically-nearest token; (2) canonical-conformance changes to a seasoned app's default theme are regressions, not polish — conform the theme _mapping_, not the shipped pixels. Consequence for Studio light QA: the token swaps slightly changed some light-theme surfaces (QB panel headers now white via `bg-panel`, some chips lighter via `bg-panel-header`) — re-QA QB in Studio light.
- **QB `text-on-accent` trap sweep (Run Query button + 5 more):** the Run Query / Stop / dropdown combo button (`query-builder__result__execute-btn__btn`) had its original `light-grey-50` text/icons mechanically mapped to `--color-text-primary` during QB Phase 1 — correct-looking in dark (both resolve light) but dark-on-green in both light themes, so the play/stop/caret icons rendered black while looking broken next to the label. On a **stable saturated fill** the text token must be `--color-text-on-accent` (light in every theme), never `text-primary/secondary`. A compiled-CSS scan (saturated palette `background` + themed `color` in the same/child block) found five more instances, all fixed: OLAP operation operator badge (yellow → stable dark `dark-grey-280`, matching its svg), OLAP window operator badge (blue-200), execution-plan `--native` view-mode name (blue-100), explorer `__label--milestoning` chip (orange-250), value-spec variable pill (pink-400). Note: Legend Query's `light-mode.scss` rules for this button target stale class names (`execute-btn__label`/`__dropdown-btn`; the component now uses `__btn`) — dead rules, left for the retirement pass. **Lesson:** the same-block scan above is cheap and worth re-running after any tokenization sweep (`background: var(--color-<saturated>-N)` + `color: var(--color-text-*)`).
- **Mapping editor source panel black-on-black (dark theme):** the source trees (`TypeTree` / `RelationTypeTree` / `FlatDataRecordTypeTree`) set no text color — historically `.source-panel`'s `panel__content` was a fixed light surface (`light-grey-100`) even in dark theme, so node labels just inherited the browser-default black. Tokenization mapped the surface to `--color-bg-elevated` (near-black in dark) but the inherited black stayed → unreadable labels for class properties / relational columns / flat-data fields. Fixed by setting `color: var(--color-text-primary)` on `.source-panel .panel__content` (covers labels and expand icons in all three source types plus the enumeration-mapping source panel). Same commit also fixed `.multiplicity-badge`, which had been mapped to `--color-text-inverted` on `--color-bg-tag` (dark-on-dark in dark theme; originally a stable light chip with translucent-dark text) → now `text-primary` on `bg-tag`, with the `--required` yellow variant keeping stable dark text (inverse trap). **Lesson for future sweeps:** when tokenizing a formerly _locked-light_ surface, check what the text color actually was — if it was inherited UA-default black rather than a declared palette color, the sweep won't see any `color:` to migrate and the surface silently breaks in dark.
