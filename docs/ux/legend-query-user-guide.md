...
| **Runtime**           | Pre-filled from the execution context and hidden by default. Turn on **Show Runtime Selector** in the settings menu (⋮) to see or change it. |

**Data spaces built on a data product.** Some execution contexts take their data
from a data product rather than a traditional database connection, and don't
name a runtime. For those, Legend Query sets up a **Lakehouse runtime** for you
automatically. The runtime field then shows your Lakehouse
**environment / warehouse** and can't be changed from the dropdown; use
**Lakehouse Runtime Configuration** in the settings menu (⋮) to adjust it — the
same settings used for data products (§3.2).
...
| Field            | Notes                                                                                                       |
| ---------------- | ----------------------------------------------------------------------------------------------------------- |
| **Data product** | Pre-filled. Can be changed when creating a new query; the dropdown lists **production** data products only. |
...
**Not available for data product queries:** four items in the **Export** menu —
**Legend DataCube**, **Curated Template Query**, **Productionized Query**, and
**DEV Service**. They appear greyed out with the tooltip _"Not supported for Data
Product or Ingest queries"_.
Running, saving, sharing, and exporting to a file all work normally.
...
**About Ingestion Data Set** in the editor menu shows details of the ingest and
data set you are querying. The same export-menu actions unavailable for data
products (§3.2) are unavailable here too.
...
**Legend DataCube** (in the Export menu) hands a **saved** query to DataCube for
...
Neither is available for data product or ingest queries (§3.2).
...
**"An export menu item is greyed out: _Not supported for Data Product or Ingest queries_."**
DataCube, template promotion, and productionizing aren't available when querying
a data product or an ingest definition. Exporting to a file still works. If you
need one of those actions, check whether a data space covers the same data.

**"The runtime field is greyed out and shows an environment / warehouse."**
This data space context reads from a data product, so Legend Query supplied a
Lakehouse runtime. Change it through **Lakehouse Runtime Configuration** in the
settings menu (⋮), not the dropdown.
