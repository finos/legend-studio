...
import { InheritanceDiagramRenderer } from 'Components/shared/diagram-viewer/InheritanceDiagramRenderer';
...
import { FaLock, FaPlus, FaTimes, FaLongArrowAltRight, FaArrowAltCircleRight, FaFire, FaArrowCircleRight } from 'react-icons/fa';
import { CustomSelectorInput } from 'Components/shared/CustomSelectorInput';
import { TEST_ID } from 'Const';
import { PRIMITIVE_TYPE, MULTIPLICITY_INFINITE } from 'MetaModelConst';
...
import { GenericTypeExplicitReference, GenericTypeReference } from 'MM/model/packageableElements/domain/GenericTypeReference';
...
const PropertyBasicEditor = observer((props: {
  ...
  const { property, _class, selectProperty, deleteProperty, isReadOnly } = props;
  const editorStore = useEditorStore();
  const isInheritedProperty = property.owner instanceof Class && property.owner !== _class;
  ...
  // Name
  const changeValue: React.ChangeEventHandler<HTMLInputElement> = event => property.setName(event.target.value);
  // Generic Type
  const [isEditingType, setIsEditingType] = useState(false);
  const propertyTypeOptions = editorStore.graphState.graph.classPropertyGenericTypeOptions;
  const propertyType = property.genericType.value.rawType;
  const propertyTypeName = getClassPropertyType(propertyType);
  const filterOption = createFilter({ ignoreCase: true, ignoreAccents: false, stringify: (option: PackageableElementSelectOption<Type>): string => option.value.path });
  const selectedPropertyType = { value: propertyType, label: propertyType.name };
  const changePropertyType = (val: PackageableElementSelectOption<Type>): void => {
    property.setGenericType(new GenericType(val.value));
    setIsEditingType(false);
  };
  // Multiplicity
  const [lowerBound, setLowerBound] = useState<string | number>(property.multiplicity.lowerBound);
  const [upperBound, setUpperBound] = useState<string | number>(property.multiplicity.upperBound ?? MULTIPLICITY_INFINITE);
  const updateMultiplicity = (lower: number | string, upper: number | string): void => {
    const lBound = typeof lower === 'number' ? lower : parseInt(lower, 10);
    const uBound = upper === MULTIPLICITY_INFINITE ? undefined : typeof upper === 'number' ? upper : parseInt(upper, 10);
    if (!isNaN(lBound) && (uBound === undefined || !isNaN(uBound))) {
      property.setMultiplicity(new Multiplicity(lBound, uBound));
    }
  };
  const changeLowerBound: React.ChangeEventHandler<HTMLInputElement> = event => {
    setLowerBound(event.target.value);
    updateMultiplicity(event.target.value, upperBound);
  };
  const changeUpperBound: React.ChangeEventHandler<HTMLInputElement> = event => {
  ...
  const openElement = (): void => {
    if (!(propertyType instanceof PrimitiveType)) {
      editorStore.openElement(propertyType instanceof Unit ? propertyType.measure : propertyType);
    }
  };
    ...
    <div className="property-basic-editor">
      {isIndirectProperty &&
        <div className="property-basic-editor__name--with-lock">
          <div className="property-basic-editor__name--with-lock__icon"><FaLock /></div>
          <span className="property-basic-editor__name--with-lock__name">{property.name}</span>
        </div>
      }
      {!isIndirectProperty &&
        <input
      ...
      {!isIndirectProperty && !isReadOnly && !isEditingType &&
        <div className={clsx('property-basic-editor__type',
          ...
          {propertyTypeName !== CLASS_PROPERTY_TYPE.PRIMITIVE && <div className="property-basic-editor__type__abbr"><ElementIcon element={propertyType} /></div>}
          <div className="property-basic-editor__type__label">{propertyType.name}</div>
          <div data-testid={TEST_ID.PROPERTY_BASIC_EDITOR__TYPE__LABEL_HOVER} className="property-basic-editor__type__label property-basic-editor__type__label--hover" onClick={(): void => setIsEditingType(true)}>Click to edit</div>
          {propertyTypeName !== CLASS_PROPERTY_TYPE.PRIMITIVE &&
            <button
              data-testid={TEST_ID.TYPE_VISIT}
      ...
      {isIndirectProperty &&
        <button
          ...
          onClick={visitOwner}
          tabIndex={-1}
          title={`Visit ${isInheritedProperty ? 'super type class' : 'association'} '${property.owner.path}'`}
        ><FaArrowAltCircleRight /></button>
      }
      {isIndirectProperty &&
      ...
      {!isIndirectProperty && !isReadOnly &&
        <button
          className={clsx('uml-element-editor__remove-btn', { 'uml-element-editor__remove-btn--hidden': isIndirectProperty })}
...
const DerivedPropertyBasicEditor = observer((props: {
  ...
  // Name
  const changeValue: React.ChangeEventHandler<HTMLInputElement> = event => derivedProperty.setName(event.target.value);
  // Generic Type
  const [isEditingType, setIsEditingType] = useState(false);
  const propertyTypeOptions = editorStore.graphState.graph.classPropertyGenericTypeOptions;
  ...
  // Multiplicity
  const [lowerBound, setLowerBound] = useState<string | number>(derivedProperty.multiplicity.lowerBound);
  const [upperBound, setUpperBound] = useState<string | number>(derivedProperty.multiplicity.upperBound ?? MULTIPLICITY_INFINITE);
  const updateMultiplicity = (lower: number | string, upper: number | string): void => {
    const lBound = typeof lower === 'number' ? lower : parseInt(lower, 10);
    const uBound = upper === MULTIPLICITY_INFINITE ? undefined : typeof upper === 'number' ? upper : parseInt(upper, 10);
    if (!isNaN(lBound) && (uBound === undefined || !isNaN(uBound))) {
  ...
  const visitOwner = (): void => editorStore.openElement(derivedProperty.owner);
  const remove = applicationStore.guaranteeSafeAction(async () => {
    await dpState.convertLambdaObjectToGrammarString(RenderStyle.STANDARD);
    deleteDerivedProperty();
  });

  return (
    <div className={clsx('derived-property-editor', { 'backdrop__element': dpState.parserError && (!isInheritedProperty && !isReadOnly) })}>
      <div className="property-basic-editor" >
        {isInheritedProperty &&
          <div className="property-basic-editor__name--with-lock">
            <div className="property-basic-editor__name--with-lock__icon"><FaLock /></div>
            <span className="property-basic-editor__name--with-lock__name">{derivedProperty.name}</span>
          </div>
        }
        {!isInheritedProperty &&
          <input
        ...
        {!isInheritedProperty && !isReadOnly &&
          <button
            className={clsx('uml-element-editor__remove-btn', { 'uml-element-editor__remove-btn--hidden': isInheritedProperty })}
            onClick={remove}
            tabIndex={-1}
            title={'Remove'}
          ><FaTimes /></button>
      ...
      <LambdaEditor
        disabled={editorState.classState.isConvertingDerivedPropertyObjects || isInheritedProperty || isReadOnly}
...
const ConstraintEditor = observer((props: {
  ...
  // Name
  const changeName: React.ChangeEventHandler<HTMLInputElement> = event => constraint.setName(event.target.value);
  // Actions
  const remove = applicationStore.guaranteeSafeAction(async () => {
    await constraintState.convertLambdaObjectToGrammarString(RenderStyle.STANDARD);
    deleteConstraint();
  });
  const visitOwner = (): void => editorStore.openElement(constraint.owner);

  return (
    <div className={clsx('constraint-editor', { 'backdrop__element': constraintState.parserError })}>
      <div className="constraint-editor__content">
        {isInheritedConstraint &&
          <div className="constraint-editor__content__name--with-lock">
            <div className="constraint-editor__content__name--with-lock__icon"><FaLock /></div>
            <span className="constraint-editor__content__name--with-lock__name">{constraint.name}</span>
          </div>
        }
        {!isInheritedConstraint &&
          <input
      ...
      <LambdaEditor
        disabled={editorState.classState.isConvertingConstraintObjects || isReadOnly || isInheritedConstraint}
        lambdaEditorState={constraintState}
        forceBackdrop={hasParserError}
        expectedType={editorStore.graphState.graph.getPrimitiveType(PRIMITIVE_TYPE.BOOLEAN)}
      />
    </div>
  );
});
...
const SuperTypeEditor = observer((props: {
  ...
  const editorStore = useEditorStore();
  // Type
  const superTypeOptions = editorStore.graphState.graph.classOptions
    .filter(classOption => classOption.value instanceof Class
  ...
  const changeType = (val: PackageableElementSelectOption<Class>): void => superType.setValue(new GenericType(val.value));
...
export const ClassFormEditor = observer((props: {
  ...
  const editorStore = useEditorStore();
  const applicationStore = useApplicationStore();
  const classHash = _class.isReadOnly ? undefined : applicationStore.notifyAndReturnAlternativeOnError(() => _class.hashCode, undefined); // attempting to read the hashCode of immutable element will throw an error
  const classState = editorState.classState;
  const isReadOnly = editorState.isReadOnly;
  const defaultType = editorStore.graphState.graph.getPrimitiveType(PRIMITIVE_TYPE.STRING);
  // Selected property
  const [selectedProperty, setSelectedProperty] = useState<Property | DerivedProperty | undefined>();
  const selectProperty = (e: Property | DerivedProperty): () => void => (): void => setSelectedProperty(e);
  // Tab
  ...
  const deleteStereotype = (val: StereotypeReference): () => void => (): void => _class.deleteStereotype(val);
  const deleteTaggedValue = (val: TaggedValue): () => void => (): void => _class.deleteTaggedValue(val);
  // Property
  const deleteProperty = (property: Property): () => void => (): void => {
    _class.deleteProperty(property);
    if (property === selectedProperty) { setSelectedProperty(undefined) }
  };
  const indirectProperties = _class.getAllProperties()
    .filter(property => !_class.properties.includes(property))
    .sort((p1, p2) => p1.name.localeCompare(p2.name))
    .sort((p1, p2) => (p1.owner === _class ? 1 : 0) - (p2.owner === _class ? 1 : 0));
  const deselectProperty = (): void => setSelectedProperty(undefined);
  // Constraints
  const deleteConstraint = (constraint: Constraint): () => void => (): void => {
    _class.deleteConstraint(constraint);
    classState.deleteConstraintState(constraint);
  };
  const inheritedConstraints = _class.getAllConstraints()
    .filter(constraint => !_class.constraints.includes(constraint));
  // Super type
  const deleteSuperType = (superType: GenericTypeReference): () => void => (): void => {
    _class.deleteSuperType(superType);
    if (superType.value.rawType instanceof Class) {
      superType.value.rawType.deleteSubClass(_class);
    }
  };
  const possibleSupertypes = editorStore.graphState.graph.classes
    .filter(superType =>
      // Exclude current class
      superType !== _class
      // Exclude super types of the class
  ...
  // Derived properties
  const indirectDerivedProperties = _class.getAllDerivedProperties()
    .filter(property => !_class.derivedProperties.includes(property))
    .sort((p1, p2) => p1.name.localeCompare(p2.name))
    .sort((p1, p2) => (p1.owner === _class ? 1 : 0) - (p2.owner === _class ? 1 : 0));
  const deleteDerivedProperty = (dp: DerivedProperty): () => void => (): void => {
    _class.deleteDerivedProperty(dp);
    classState.deleteDerivedPropertyState(dp);
    if (dp === selectedProperty) { setSelectedProperty(undefined) }
  };
  // Add button
  let addButtonTitle = '';
  switch (selectedTab) {
    case UML_EDITOR_TAB.PROPERTIES: addButtonTitle = 'Add property'; break;
    case UML_EDITOR_TAB.DERIVED_PROPERTIES: addButtonTitle = 'Add derived property'; break;
    case UML_EDITOR_TAB.CONSTRAINTS: addButtonTitle = 'Add constraint'; break;
    case UML_EDITOR_TAB.SUPER_TYPES: addButtonTitle = 'Add super type'; break;
  ...
  const isAddButtonDisabled = isReadOnly || (selectedTab === UML_EDITOR_TAB.SUPER_TYPES && !possibleSupertypes.length);
  const add = (): void => {
    if (!isReadOnly) {
      ...
      } else if (selectedTab === UML_EDITOR_TAB.SUPER_TYPES && possibleSupertypes.length) {
        _class.addSuperType(GenericTypeExplicitReference.create(new GenericType(possibleSupertypes[0])));
        possibleSupertypes[0].addSubClass(_class);
      } else if (selectedTab === UML_EDITOR_TAB.TAGGED_VALUES) {
        _class.addTaggedValue(TaggedValue.createStub(Tag.createStub(Profile.createStub())));
      } else if (selectedTab === UML_EDITOR_TAB.STEREOTYPES) {
        _class.addStereotype(StereotypeExplicitReference.create(Stereotype.createStub(Profile.createStub())));
      }
    }
  };
  // Drag and Drop
  const handleDropProperty = (item: UMLEditorElementDropTarget): void => {
    if (!isReadOnly && item.data.packageableElement instanceof Type) {
      _class.addProperty(Property.createStub(item.data.packageableElement, _class));
    }
  };
  const [{ isPropertyDragOver }, dropPropertyRef] = useDrop({
    accept: [DND_TYPE.PROJECT_EXPLORER_CLASS, DND_TYPE.PROJECT_EXPLORER_ENUMERATION],
    drop: (item: ElementDragSource): void => handleDropProperty(item),
    collect: monitor => ({ isPropertyDragOver: monitor.isOver({ shallow: true }) }),
  });
  const handleDropDerivedProperty = (item: UMLEditorElementDropTarget): void => {
    if (!isReadOnly && item.data.packageableElement instanceof Type) {
      const dp = DerivedProperty.createStub(item.data.packageableElement, _class);
      _class.addDerivedProperty(dp);
      classState.addDerivedPropertyState(dp);
    }
  };
  const [{ isDerivedPropertyDragOver }, dropDerivedPropertyRef] = useDrop({
    accept: [DND_TYPE.PROJECT_EXPLORER_CLASS, DND_TYPE.PROJECT_EXPLORER_ENUMERATION],
    drop: (item: ElementDragSource): void => handleDropDerivedProperty(item),
    collect: monitor => ({ isDerivedPropertyDragOver: monitor.isOver({ shallow: true }) }),
  });
  const handleDropSuperType = (item: UMLEditorElementDropTarget): void => {
    ...
    if (
      ...
      _class.addSuperType(GenericTypeExplicitReference.create(new GenericType(element)));
  ...
  const [{ isSuperTypeDragOver }, dropSuperTypeRef] = useDrop({
    ...
    collect: monitor => ({ isSuperTypeDragOver: monitor.isOver({ shallow: true }) }),
  });
  const handleDropTaggedValue = (item: UMLEditorElementDropTarget): void => {
    if (!isReadOnly && item.data.packageableElement instanceof Profile) {
      _class.addTaggedValue(TaggedValue.createStub(Tag.createStub(item.data.packageableElement)));
    }
  };
  ...
  const handleDropStereotype = (item: UMLEditorElementDropTarget): void => {
    if (!isReadOnly && item.data.packageableElement instanceof Profile) {
      _class.addStereotype(StereotypeExplicitReference.create(Stereotype.createStub(item.data.packageableElement)));
    }
  };
  ...
  useEffect(() => { onHashChange?.() }, [_class, classHash, onHashChange]);
  // Decorate (add/remove states for derived properties/constraints) and convert lambda objects
  useEffect(() => {
    classState.decorate();
    classState.convertConstraintObjects().catch(applicationStore.alertIllegalUnhandledError);
    classState.convertDerivedPropertyObjects().catch(applicationStore.alertIllegalUnhandledError);
  }, [applicationStore, classState]);

  return (
    <div data-testid={TEST_ID.CLASS_FORM_EDITOR} className="uml-element-editor class-form-editor">
      <SplitPane split="horizontal" primary="second" size={selectedProperty ? 250 : 0} minSize={250} maxSize={-56}>
        <div className="panel">
          <div className="panel__header">
            <div className="panel__header__title">
              ...
              <div className="panel__header__title__content">{_class.name}</div>
            </div>
            <div className="panel__header__actions">
              {_class.generationParentElement &&
                ...
                >
                  ...
                  <div className="uml-element-editor__header__generation-origin__parent-name">{_class.generationParentElement.name}</div>
          ...
          <div data-testid={TEST_ID.UML_ELEMENT_EDITOR__TABS_HEADER} className="panel__header uml-element-editor__tabs__header">
            <div className="uml-element-editor__tabs">
              {tabs.map(tab =>
                <div key={tab} onClick={changeTab(tab)} className={clsx('uml-element-editor__tab', { 'uml-element-editor__tab--active': tab === selectedTab })}>
                  {prettyCONSTName(tab)}
                </div>
              )}
            </div>
          ...
          <div className={clsx('panel__content', { 'panel__content--with-backdrop-element': selectedTab === UML_EDITOR_TAB.DERIVED_PROPERTIES || selectedTab === UML_EDITOR_TAB.CONSTRAINTS })}>
            {selectedTab === UML_EDITOR_TAB.PROPERTIES &&
              <div ref={dropPropertyRef} className={clsx('panel__content__lists', { 'panel__content__lists--dnd-over': isPropertyDragOver && !isReadOnly })}>
                {_class.properties.concat(indirectProperties).map(property =>
                  <PropertyBasicEditor key={property.uuid} property={property} _class={_class} deleteProperty={deleteProperty(property)} selectProperty={selectProperty(property)} isReadOnly={isReadOnly} />
                )}
              </div>
            }
            {selectedTab === UML_EDITOR_TAB.DERIVED_PROPERTIES &&
              <div ref={dropDerivedPropertyRef} className={clsx('panel__content__lists', { 'panel__content__lists--dnd-over': isDerivedPropertyDragOver && !isReadOnly })}>
                {_class.derivedProperties.concat(indirectDerivedProperties).filter((dp): dp is DerivedProperty => Boolean(classState.getNullableDerivedPropertyState(dp))).map(dp =>
                  <DerivedPropertyBasicEditor key={dp.uuid} derivedProperty={dp} _class={_class} editorState={editorState} deleteDerivedProperty={deleteDerivedProperty(dp)} selectDerivedProperty={selectProperty(dp)} isReadOnly={isReadOnly} />
                )}
              </div>
            }
            {selectedTab === UML_EDITOR_TAB.CONSTRAINTS &&
              <div className="panel__content__lists">
                {_class.constraints.concat(inheritedConstraints).filter((constraint): constraint is Constraint => Boolean(classState.getNullableConstraintState(constraint))).map(constraint =>
                  <ConstraintEditor key={constraint.uuid} constraint={constraint} _class={_class} editorState={editorState} deleteConstraint={deleteConstraint(constraint)} isReadOnly={isReadOnly} />
                )}
              </div>
            }
            {selectedTab === UML_EDITOR_TAB.SUPER_TYPES &&
              <div ref={dropSuperTypeRef} className={clsx('panel__content__lists', { 'panel__content__lists--dnd-over': isSuperTypeDragOver && !isReadOnly })}>
                {_class.generalizations.map(superType =>
                  <SuperTypeEditor key={superType.value.uuid} superType={superType} _class={_class} deleteSuperType={deleteSuperType(superType)} isReadOnly={isReadOnly} />
                )}
              </div>
            }
        ...
        {selectedProperty ? <PropertyEditor property={selectedProperty} deselectProperty={deselectProperty} isReadOnly={isReadOnly} /> : <div />}
      </SplitPane>
    </div>
  );
});

export const ClassEditor = observer((props: {
  _class: Class;
}) => {
  const { _class } = props;
  const editorStore = useEditorStore();
  const applicationStore = useApplicationStore();
  const classHash = _class.isReadOnly ? undefined : applicationStore.notifyAndReturnAlternativeOnError(() => _class.hashCode, undefined); // attempting to read the hashCode of immutable element will throw an error
  const [diagramRenderer, setDiagramRenderer] = useState<InheritanceDiagramRenderer>();
  ...
  return (
    <SplitPane split="vertical" onDragFinished={resizeDiagram} defaultSize={500} minSize={450} maxSize={-450}>
      <ReactResizeDetector
        handleHeight={true}
        handleWidth={true}
        ...
        refreshRate={50}
      >
        <div className="class-editor__diagram-viewer">
          <div ref={canvas}
