...
import { StereotypeSelectOption } from 'MM/model/packageableElements/domain/Stereotype';
...
export const StereotypeSelector = observer((props: {
  ...
  // Profile
  const profileOptions = editorStore.graphState.graph.profileOptions.filter(p => p.value.stereotypes.length);
  const filterOption = createFilter({ ignoreCase: true, ignoreAccents: false, stringify: (option: PackageableElementSelectOption<Profile>): string => option.value.path });
  const [selectedProfile, setSelectedProfile] = useState<PackageableElementSelectOption<Profile>>({ value: stereotype.value.owner, label: stereotype.value.owner.name });
  ...
  const stereotypeFilterOption = createFilter({ ignoreCase: true, ignoreAccents: false, stringify: (option: StereotypeSelectOption): string => option.label });
  const selectedStereotype = { value: stereotype.value, label: stereotype.value.value };
  const updateStereotype = (val: StereotypeSelectOption): void => stereotype.setValue(val.value);
  return (
