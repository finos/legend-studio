...
const TagBasicEditor = observer((props: {
  tag: Tag;
  deleteValue: () => void;
  isReadOnly: boolean;
}) => {
  const { tag, deleteValue, isReadOnly } = props;
  const changeValue: React.ChangeEventHandler<HTMLInputElement> = event => tag.setValue(event.target.value);
...
const StereotypeBasicEditor = observer((props: {
  stereotype: Stereotype;
  deleteStereotype: () => void;
  isReadOnly: boolean;
}) => {
  const { stereotype, deleteStereotype, isReadOnly } = props;
  const changeValue: React.ChangeEventHandler<HTMLInputElement> = event => stereotype.setValue(event.target.value);
...
export const ProfileEditor = observer((props: {
  profile: Profile;
}) => {
  const { profile } = props;
  ...
  const changeTab = (tab: UML_EDITOR_TAB): () => void => (): void => editorState.setSelectedTab(tab);
  // Tagged value and Stereotype
  let addButtonTitle = '';
  switch (selectedTab) {
    case UML_EDITOR_TAB.TAGS: addButtonTitle = 'Add tagged value'; break;
  ...
  const deleteStereotype = (val: Stereotype): () => void => (): void => profile.deleteStereotype(val);
  const deleteTag = (val: Tag): () => void => (): void => profile.deleteTag(val);
  return (
          ...
          {selectedTab === UML_EDITOR_TAB.TAGS &&
            <div className="panel__content__lists">
              {profile.tags.map(tag =>
                <TagBasicEditor key={tag.uuid} tag={tag} deleteValue={deleteTag(tag)} isReadOnly={isReadOnly} />
              )}
            </div>
          }
          {selectedTab === UML_EDITOR_TAB.STEREOTYPES &&
            <div className="panel__content__lists">
              {profile.stereotypes.map(stereotype =>
                <StereotypeBasicEditor key={stereotype.uuid} stereotype={stereotype} deleteStereotype={deleteStereotype(stereotype)} isReadOnly={isReadOnly} />
              )}
            </div>
          }
        </div>
