...
import { editor as monacoEditorAPI, languages as monacoLanguagesAPI, Range, IDisposable } from 'monaco-editor';
import { TAB_SIZE, EDITOR_THEME, EDITOR_LANGUAGE } from 'Stores/EditorConfig';
import ReactResizeDetector from 'react-resize-detector';
import { disposeEditor, disableEditorHotKeys, baseTextEditorSettings, moveToPosition, setErrorMarkers, revealError } from 'Utilities/TextEditorUtil';
import { EntityChangeConflict } from 'SDLC/entity/EntityChangeConflict';
import { EntityChangeConflictEditorState, ENTITY_CHANGE_CONFLICT_EDITOR_VIEW_MODE, MergeEditorComparisonViewInfo, MergeConflict } from 'Stores/editor-state/entity-diff-editor-state/EntityChangeConflictEditorState';
import { IllegalStateError, shallowStringify, debounce, isNonNullable } from 'Utilities/GeneralUtil';
import { hashObject } from 'Utilities/HashUtil';
import { FaArrowDown, FaArrowUp } from 'react-icons/fa';
import { CustomSelectorInput } from 'Components/shared/CustomSelectorInput';
import { TextDiffView } from 'Components/shared/DiffView';
...
const getConflictSummaryText = (conflictEditorState: EntityChangeConflictEditorState): string => {
  ...
  if (!conflictEditorState.baseEntity && conflictEditorState.currentChangeEntity && conflictEditorState.incomingChangeEntity) {
    // [CREATE - CREATE]
    return `Entity is created in both your changes and their changes`;
  } else if (conflictEditorState.baseEntity) {
    if (!conflictEditorState.currentChangeEntity && conflictEditorState.incomingChangeEntity) {
      // [DELETE - MODIFY]
      return `Entity is deleted in your changes but modified in their changes`;
    } else if (conflictEditorState.currentChangeEntity && !conflictEditorState.incomingChangeEntity) {
      // [MODIFY - DELETE]
      return `Entity is modified in your changes but deleted in their changes`;
    } else if (conflictEditorState.currentChangeEntity && conflictEditorState.incomingChangeEntity) {
      // [MODIFY - MODIFY]
      if (hashObject(conflictEditorState.currentChangeEntity) === hashObject(conflictEditorState.incomingChangeEntity)) {
...
export const EntityChangeConflictSideBarItem = observer((props: {
  ...
  return (
    <button
      ...
      className={clsx('side-bar__panel__item', { 'side-bar__panel__item--selected': isSelected })}
      tabIndex={-1}
      title={`${conflict.entityPath} \u2022 Has merge conflict(s)\n${conflict.conflictReason}`}
      onClick={openConflict}
    >
      <div className="diff-panel__item__info">
        <span className="diff-panel__item__info-name diff-panel__item__info-name--conflict">{conflict.entityName}</span>
        <span className="diff-panel__item__info-path">{conflict.entityPath}</span>
      </div>
      <div className="diff-panel__item__type diff-panel__item__type--conflict">C</div>
    </button>
  );
});
...
const MergeConflictEditor = observer((props: {
  ...
  const mergeConflictResolutionCodeLensDisposer = useRef<IDisposable | undefined>(undefined);
  ...
  // cursor
  const onDidChangeCursorPositionEventDisposer = useRef<IDisposable | undefined>(undefined);
  const onDidBlurEditorTextEventDisposer = useRef<IDisposable | undefined>(undefined);
  const onDidFocusEditorTextEventDisposer = useRef<IDisposable | undefined>(undefined);
  ...
  if (editor) {
    ...
    onDidChangeCursorPositionEventDisposer.current = editor.onDidChangeCursorPosition(event => {
      ...
      if (editor.hasTextFocus()) { conflictEditorState.setCurrentMergeEditorLine(event.position.lineNumber) }
    ...
    const selectCurrentChangeCommandId = editor.addCommand(-1, (baseArg, conflict: MergeConflict) => conflictEditorState.acceptCurrentChange(conflict));
    const selectIncomingChangeCommandId = editor.addCommand(-1, (baseArg, conflict: MergeConflict) => conflictEditorState.acceptIncomingChange(conflict));
    const selectBothChangesCommandId = editor.addCommand(-1, (baseArg, conflict: MergeConflict) => conflictEditorState.acceptBothChanges(conflict));
    const rejectBothChangesCommandId = editor.addCommand(-1, (baseArg, conflict: MergeConflict) => conflictEditorState.rejectBothChanges(conflict));
    ...
    mergeConflictResolutionCodeLensDisposer.current = monacoLanguagesAPI.registerCodeLensProvider(EDITOR_LANGUAGE.PURE, {
      provideCodeLenses: (model, token) => ({
        lenses: conflictEditorState.mergeConflicts.flatMap(conflict => [
          !isReadOnly && selectCurrentChangeCommandId ? {
            range: { startLineNumber: conflict.startHeader, endLineNumber: conflict.startHeader, startColumn: 1, endColumn: 1 },
            command: {
        ...
        dispose: (): void => { /** no need to do anything on disposal */ }
      }),
    ...
    decorations.current = editor.deltaDecorations(decorations.current, conflictEditorState.mergeConflicts.flatMap(conflict => {
  ...
  useEffect(() => {
    if (editor && conflictEditorState.currentMergeEditorConflict !== undefined) {
      moveToPosition(editor, conflictEditorState.currentMergeEditorConflict.startHeader, 1);
    }
  }, [editor, conflictEditorState, conflictEditorState.currentMergeEditorConflict]);
...
const getMergeEditorViewModeOption = (mode: ENTITY_CHANGE_CONFLICT_EDITOR_VIEW_MODE, modeComparisonViewInfo: MergeEditorComparisonViewInfo): { label: React.ReactNode, value: ENTITY_CHANGE_CONFLICT_EDITOR_VIEW_MODE } => ({
  value: mode,
  label: (
    <div className="entity-change-conflict-editor__header__action__view-dropdown__option">
      <div className={`entity-change-conflict-editor__header__action__view-dropdown__option__label entity-change-conflict-editor__header__action__view-dropdown__option__label--${mode.toLowerCase()}`}>{modeComparisonViewInfo.label}</div>
      {mode !== ENTITY_CHANGE_CONFLICT_EDITOR_VIEW_MODE.MERGE_VIEW &&
        <div className="entity-change-conflict-editor__header__action__view-dropdown__option__summary">
          <div className="entity-change-conflict-editor__header__action__view-dropdown__option__summary__revision">{getPrettyLabelForRevision(modeComparisonViewInfo.fromRevision)}</div>
          <div className="entity-change-conflict-editor__header__action__view-dropdown__option__summary__icon"><MdCompareArrows /></div>
          <div className="entity-change-conflict-editor__header__action__view-dropdown__option__summary__revision">{getPrettyLabelForRevision(modeComparisonViewInfo.toRevision)}</div>
        </div>
      }
    </div>
  ),
...
export const EntityChangeConflictEditor = observer(() => {
  ...
  const conflictEditorState = editorStore.getCurrentEditorState(EntityChangeConflictEditorState);
  const isReadOnly = conflictEditorState.isReadOnly;
  ...
  // header actions
  const goToPreviousConflict = (): void => { if (conflictEditorState.previousConflict) { conflictEditorState.setCurrentMergeEditorConflict(conflictEditorState.previousConflict) } };
  const goToNextConflict = (): void => { if (conflictEditorState.nextConflict) { conflictEditorState.setCurrentMergeEditorConflict(conflictEditorState.nextConflict) } };
  // resolutions
  const markAsResolved = applicationStore.guaranteeSafeAction(() => conflictEditorState.markAsResolved());
  const useTheirs = applicationStore.guaranteeSafeAction(() => conflictEditorState.useIncomingChanges());
  const useYours = applicationStore.guaranteeSafeAction(() => conflictEditorState.useCurrentChanges());
  // mode
  ...
  const onModeChange = (val: { value: ENTITY_CHANGE_CONFLICT_EDITOR_VIEW_MODE }): void => conflictEditorState.setCurrentMode(val.value);

  useEffect(() => { conflictEditorState.refresh().catch(applicationStore.alertIllegalUnhandledError) }, [applicationStore, conflictEditorState]);

  useEffect(() => {
    debouncedUpdateMergeConflicts.cancel();
    debouncedUpdateMergeConflicts();
  }, [conflictEditorState, conflictEditorState.mergedText, debouncedUpdateMergeConflicts]);

  // reset transient merge editor states (e.g. cursor position, current merge conflict, etc.) as we exit the tab
  useEffect(() => (): void => conflictEditorState.resetMergeEditorStateOnLeave(), [conflictEditorState]);
      ...
      <div className="entity-change-conflict-editor__header">
        <div className="entity-change-conflict-editor__header__info">
          <div className="entity-change-conflict-editor__header__info__tag">
            <div className="entity-change-conflict-editor__header__info__tag__text">{isReadOnly ? 'Merge preview' : conflictEditorState.mergeSucceeded ? 'Merged successfully' : 'Merged with conflict(s)'}</div>
          </div>
          <div className="entity-change-conflict-editor__header__info__comparison-summary">{getConflictSummaryText(conflictEditorState)}</div>
        </div>
        <div className="entity-change-conflict-editor__header__info__tag">
          <div className="entity-change-conflict-editor__header__info__tag__text">{conflictEditorState.mergeConflicts.length} conflict(s)</div>
        </div>
      </div>
      <div className="entity-change-conflict-editor__header__actions">
        <div className="entity-change-conflict-editor__header__actions__main">
          ...
          <CustomSelectorInput
            ...
            options={Object.values(ENTITY_CHANGE_CONFLICT_EDITOR_VIEW_MODE).map(mode => getMergeEditorViewModeOption(mode, conflictEditorState.getModeComparisonViewInfo(mode)))}
            isSearchable={false}
            onChange={onModeChange}
            value={getMergeEditorViewModeOption(currentMode, conflictEditorState.getModeComparisonViewInfo(currentMode))}
            darkMode={true}
          />
        </div>
      </div>
      <div className={clsx('entity-change-conflict-editor__content', { 'entity-change-conflict-editor__content--read-only': isReadOnly })}>
        {currentMode === ENTITY_CHANGE_CONFLICT_EDITOR_VIEW_MODE.MERGE_VIEW && <MergeConflictEditor conflictEditorState={conflictEditorState} />}
        {currentMode !== ENTITY_CHANGE_CONFLICT_EDITOR_VIEW_MODE.MERGE_VIEW &&
          <TextDiffView language={EDITOR_LANGUAGE.PURE} from={currentModeComparisonViewInfo.fromGrammarText ?? ''} to={currentModeComparisonViewInfo.toGrammarText ?? ''} />
        }
      </div>
      {!isReadOnly &&
