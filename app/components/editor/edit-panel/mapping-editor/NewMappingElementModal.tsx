...
import { BASIC_SET_IMPLEMENTATION_TYPE } from 'MM/model/packageableElements/mapping/SetImplementation';
...
export const NewMappingElementModal = observer(() => {
  ...
  const handleIdChange: React.ChangeEventHandler<HTMLInputElement> = event => setId(event.target.value);
  const mapping = mappingEditorState.mapping;
  const mappingIds = mapping.getAllMappingElements().map(mappingElement => mappingElement.id.value);
  const isMappingIdUnique = !mappingIds.includes(id);
  const showId = spec?.target && mappingIds.includes(fromElementPathToMappingElementId(spec.target.path));

  // Target
  const targetSelectorRef = useRef<SelectComponent>(null);
  const options: PackageableElementSelectOption<PackageableElement>[] = [...editorStore.graphState.graph.enumerationOptions, ...editorStore.graphState.graph.associationOptions, ...editorStore.graphState.graph.classOptions]
    .sort(compareLabelFn);
  ...
  const selectedOption = spec?.target ? { label: spec.target.name, value: spec.target.path } : null;
  const handleTargetChange = (val: PackageableElementSelectOption<PackageableElement> | null): void => {
    mappingEditorState.createMappingElement({ target: val?.value, showTarget: true, openInAdjacentTab: Boolean(spec?.openInAdjacentTab) });
    const suggestedId = val?.value ? fromElementPathToMappingElementId(val.value.path) : '';
  ...
  const classMappingTypeOptions = [{ value: BASIC_SET_IMPLEMENTATION_TYPE.INSTANCE, label: 'Instance' }, { value: BASIC_SET_IMPLEMENTATION_TYPE.OPERATION, label: 'Operation' }];
  const initialClassMappingType = classMappingTypeOptions[0];
  const [classMappingType, setClassMappingType] = useState<ClassMappingSubTypeOption | null>(initialClassMappingType);
  const changeClassMappingType = (val: ClassMappingSubTypeOption): void => setClassMappingType(val);

  // Dialog actions
  const handleClose = (): void => mappingEditorState.setNewMappingElementSpec(undefined);
  const handleEnter = (): void => {
    if (spec) {
      const suggestedId = spec.target ? fromElementPathToMappingElementId(spec.target.path) : '';
      setId(suggestedId && !mappingIds.includes(suggestedId) ? suggestedId : '');
  ...
  // Submit button
  const disableCreateButton = !spec || !spec.target || !isMappingIdUnique || !id || (spec.target instanceof Class && !classMappingType);
      ...
      if (id && spec.target) {
        ...
        if (spec.target instanceof Class) {
          if (classMappingType?.value) {
            newMapppingElement = mapping.createClassMapping(id, spec.target, classMappingType.value);
          }
        } else if (spec.target instanceof Enumeration) {
          newMapppingElement = mapping.createEnumerationMapping(id, spec.target, editorStore.graphState.graph.getPrimitiveType(PRIMITIVE_TYPE.STRING));
        ...
        if (newMapppingElement) {
          mappingEditorState.openMappingElement(newMapppingElement, spec.openInAdjacentTab);
        }
      }
      handleClose();
    }
  ...
  const titleText = spec
    ? spec.showTarget
      ...
      : `Create New ${spec.target instanceof Class ? 'Class' : spec.target instanceof Enumeration ? 'Enumeration' : 'Association'} Mapping ${spec.target ? `for ${spec.target.name}` : ''}`
    : undefined;
  ...
  return (
    ...
    >
      {spec &&
        <form onSubmit={onSubmit} className="modal search-modal new-mapping-element-modal">
          ...
          {spec.target instanceof Class &&
            <CustomSelectorInput
              ...
              disabled={config.features.BETA__demoMode}
              placeholder={`Choose a class mapping type`}
            />
          }
          <button className="btn btn--primary u-pull-right" disabled={disableCreateButton} color="primary">Create</button>
        </form>
      }
      {!spec && <div></div>}
    </Dialog>
