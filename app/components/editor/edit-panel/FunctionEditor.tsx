...
import { FunctionEditorState, FUNCTION_SPEC_TAB } from 'Stores/editor-state/element-editor-state/FunctionEditorState';
import { UMLEditorElementDropTarget, DND_TYPE, ElementDragSource } from 'Utilities/DnDUtil';
...
export const getFunctionParameterType = (type: Type): FUNCTION_PARAMETER_TYPE => {
  ...
  throw new UnsupportedOperationError(`Unsupported function parameter type '${type.constructor.name}'`);
};
...
const ParameterBasicEditor = observer((props: {
  ...
  // Name
  const changeValue: React.ChangeEventHandler<HTMLInputElement> = event => parameter.setName(event.target.value);
  // Type
  const [isEditingType, setIsEditingType] = useState(false);
  const typeOptions = editorStore.graphState.graph.classPropertyGenericTypeOptions;
  ...
  const changeType = (val: PackageableElementSelectOption<Type>): void => { parameter.setType(val.value); setIsEditingType(false) };
  ...
  // Multiplicity
  const [lowerBound, setLowerBound] = useState<string | number>(parameter.multiplicity.lowerBound);
  const [upperBound, setUpperBound] = useState<string | number>(parameter.multiplicity.upperBound ?? MULTIPLICITY_INFINITE);
  ...
  return (
    <div className="property-basic-editor">
      {isReadOnly && <div className="property-basic-editor__lock"><FaLock /></div>}
      <input
      ...
      {!isReadOnly && !isEditingType &&
        <div className={clsx('property-basic-editor__type',
          ...
          {typeName !== FUNCTION_PARAMETER_TYPE.PRIMITIVE && <div className="property-basic-editor__type__abbr"><ElementIcon element={paramType} /></div>}
          <div className="property-basic-editor__type__label">{paramType.name}</div>
          <div className="property-basic-editor__type__label property-basic-editor__type__label--hover" onClick={(): void => setIsEditingType(true)}>Click to edit</div>
...
const ReturnTypeEditor = observer((props: {
  ...
  const changeType = (val: PackageableElementSelectOption<Type>): void => { functionElement.setReturnType(val.value); setIsEditingType(false) };

  const openElement = (): void => {
    if (!(returnType.value instanceof PrimitiveType)) {
      editorStore.openElement(returnType.value instanceof Unit ? returnType.value.measure : returnType.value);
    }
  };
  // Multiplicity
  const [lowerBound, setLowerBound] = useState<string | number>(returnMultiplicity.lowerBound);
  const [upperBound, setUpperBound] = useState<string | number>(returnMultiplicity.upperBound ?? MULTIPLICITY_INFINITE);
  const updateMultiplicity = (lower: number | string, upper: number | string): void => {
    const lBound = typeof lower === 'number' ? lower : parseInt(lower, 10);
    const uBound = upper === MULTIPLICITY_INFINITE ? undefined : typeof upper === 'number' ? upper : parseInt(upper, 10);
    if (!isNaN(lBound) && (uBound === undefined || !isNaN(uBound))) {
    ...
    <div className="function-editor__return__type-editor">
      ...
      {!isReadOnly && !isEditingType &&
        <div className={clsx('property-basic-editor__type',
          ...
          {typeName !== FUNCTION_PARAMETER_TYPE.PRIMITIVE && <div className="property-basic-editor__type__abbr"><ElementIcon element={returnType.value} /></div>}
          <div className="property-basic-editor__type__label">{returnType.value.name}</div>
          <div className="property-basic-editor__type__label property-basic-editor__type__label--hover" onClick={(): void => setIsEditingType(true)}>Click to edit</div>
          {typeName !== FUNCTION_PARAMETER_TYPE.PRIMITIVE &&
            <button
              data-testid={TEST_ID.TYPE_VISIT}
...
export const FunctionMainEditor = observer((props: {
  ...
  // Parameters
  const addParameter = (): void => { functionElement.addParameter(VariableExpression.createStub(defaultType)) };
  const deleteParameter = (val: VariableExpression): () => void => (): void => { functionElement.deleteParameter(val) };
  const handleDropParameter = (item: UMLEditorElementDropTarget): void => {
    if (!isReadOnly && item.data.packageableElement instanceof Type) {
      functionElement.addParameter(VariableExpression.createStub(item.data.packageableElement));
    }
  };
  const [{ isParameterDragOver }, dropParameterRef] = useDrop({
    ...
    collect: monitor => ({ isParameterDragOver: monitor.isOver({ shallow: true }) }),
  });
    ...
    <div className="panel__content function-editor__element">
      <div className="function-editor__element__item">
        <div className="function-editor__element__item__header">
          <div className="function-editor__element__item__header__title">PARAMETERS</div>
          <button
        ...
        <div ref={dropParameterRef} className={clsx('function-editor__element__item__content', { 'panel__content__lists--dnd-over': isParameterDragOver && !isReadOnly })}>
          {functionElement.parameters.map(param =>
            <ParameterBasicEditor key={param.uuid} parameter={param} deleteParameter={deleteParameter(param)} isReadOnly={isReadOnly} />
          )}
        </div>
      </div>
      <div className="function-editor__element__item">
        <div className="function-editor__element__item__header">
          <div className="function-editor__element__item__header__title" >LAMBDA</div>
          <ReturnTypeEditor functionElement={functionElement} isReadOnly={isReadOnly} />
        </div>
        <div className={clsx('function-editor__element__item__content', { 'backdrop__element': Boolean(functionEditorState.functionBodyEditorState.parserError) })}>
          <LambdaEditor
            className={'function-editor__element__lambda-editor'}
            disabled={lambdaEditorState.isConvertingFunctionBodyToString || isReadOnly}
...
export const FunctionEditor = observer(() => {
  ...
  let addButtonTitle = '';
  switch (selectedTab) {
    case FUNCTION_SPEC_TAB.TAGGED_VALUES: addButtonTitle = 'Add stereotype'; break;
    case FUNCTION_SPEC_TAB.STEREOTYPES: addButtonTitle = 'Add tagged value'; break;
    default: break;
  }
  ...
  const add = (): void => {
    if (!isReadOnly) {
      if (selectedTab === FUNCTION_SPEC_TAB.TAGGED_VALUES) {
        functionElement.addTaggedValue(TaggedValue.createStub(Tag.createStub(Profile.createStub())));
      } else if (selectedTab === FUNCTION_SPEC_TAB.STEREOTYPES) {
        functionElement.addStereotype(StereotypeExplicitReference.create(Stereotype.createStub(Profile.createStub())));
      }
    }
  };
  const handleDropTaggedValue = (item: UMLEditorElementDropTarget): void => {
    if (!isReadOnly && item.data.packageableElement instanceof Profile) {
      functionElement.addTaggedValue(TaggedValue.createStub(Tag.createStub(item.data.packageableElement)));
    }
  };
  ...
  const handleDropStereotype = (item: UMLEditorElementDropTarget): void => {
    if (!isReadOnly && item.data.packageableElement instanceof Profile) {
      functionElement.addStereotype(StereotypeExplicitReference.create(Stereotype.createStub(item.data.packageableElement)));
    }
  };
  ...
  const deleteStereotype = (val: StereotypeReference): () => void => (): void => functionElement.deleteStereotype(val);
  const deleteTaggedValue = (val: TaggedValue): () => void => (): void => functionElement.deleteTaggedValue(val);
  const changeTab = (tab: FUNCTION_SPEC_TAB): () => void => (): void => functionEditorState.setSelectedTab(tab);

  useEffect(() => { functionEditorState.functionBodyEditorState.convertLambdaObjectToGrammarString(RenderStyle.STANDARD, true).catch(applicationStore.alertIllegalUnhandledError) }, [applicationStore, functionEditorState]);
      ...
      <div className="panel">
        <div className="panel__header">
          <div className="panel__header__title">
            ...
            <div className="panel__header__title__content">{functionElement.name}</div>
          </div>
        </div>
        <div className="panel__header function-editor__tabs__header">
          <div className="function-editor__tabs">
            {Object.values(FUNCTION_SPEC_TAB).map(tab => <div key={tab} onClick={changeTab(tab)}
              className={clsx('function-editor__tab', { 'function-editor__tab--active': tab === selectedTab })}>{prettyCONSTName(tab)}</div>)}
          </div>
        ...
        {selectedTab === FUNCTION_SPEC_TAB.GENERAL
          ? <FunctionMainEditor functionEditorState={functionEditorState} functionElement={functionElement} isReadOnly={isReadOnly} />
