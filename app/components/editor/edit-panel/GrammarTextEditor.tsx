...
import { revealError, setErrorMarkers, disposeEditor, baseTextEditorSettings, disableEditorHotKeys } from 'Utilities/TextEditorUtil';
...
export const GrammarTextEditorHeaderTabContextMenu = observer((props: {}, ref: React.Ref<HTMLDivElement>) => {
  const editorStore = useEditorStore();
  const applicationStore = useApplicationStore();
  const leaveTextMode = applicationStore.guaranteeSafeAction(() => editorStore.toggleTextMode());

  return (
    <div ref={ref} className="edit-panel__header__tab__context-menu">
      <button className="edit-panel__header__tab__context-menu__item" onClick={leaveTextMode}>Leave Text Mode</button>
    </div>
  );
}, { forwardRef: true });
    ...
    if (!editor && textInput.current) {
      ...
      editor.onKeyDown(event => {
        if (event.keyCode === KeyCode.F9) {
          ...
          editorStore.graphState.globalCompileInTextMode().catch(applicationStore.alertIllegalUnhandledError);
    ...
    if (editor && currentElementLabelRegexString) {
      ...
      if (editorModel) {
        ...
        if (Array.isArray(match) && match.length) {
          ...
          editor.focus();
          editor.revealPositionInCenter({ lineNumber: range.startLineNumber, column: range.startColumn });
          editor.setPosition({ column: range.startColumn, lineNumber: range.startLineNumber });
        }
      }
    }
    ...
    <div className="panel edit-panel">
      <ContextMenu className="panel__header edit-panel__header" disabled={true}>
        <div className="edit-panel__header__tabs">
          ...
          >
            <div className="edit-panel__header__tab__icon"><FaUserSecret /></div>
