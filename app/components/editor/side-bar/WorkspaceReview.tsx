...
import { FaInfoCircle, FaTimes, FaPlus, FaExternalLinkSquareAlt } from 'react-icons/fa';
...
export const WorkspaceReviewDiffs = observer(() => {
  ...
  const openChange = (diff: EntityDiff): () => void => (): void => workspaceReviewState.openReviewChange(diff);
    ...
    <div className="panel side-bar__panel workspace-review__diffs">
      <div className="panel__header">
        <div className="panel__header__title">
          ...
          <div className="side-bar__panel__title__info" title="All changes made in the workspace since the revision the workspace is created">
            <FaInfoCircle />
          </div>
        </div>
...
export const WorkspaceReview = observer(() => {
  ...
  const editReviewTitle: React.ChangeEventHandler<HTMLInputElement> = event => {
    if (!workspaceReview) {
  ...
  const refresh = (): void => {
    workspaceReviewState.refreshWorkspaceChanges().catch(applicationStore.alertIllegalUnhandledError);
    workspaceReviewState.fetchCurrentWorkspaceReview().catch(applicationStore.alertIllegalUnhandledError);
  };
  const closeReview = (): void => {
    workspaceReviewState.setReviewTitle('');
    workspaceReviewState.closeWorkspaceReview().catch(applicationStore.alertIllegalUnhandledError);
  };
  const commitReview = (): void => {
    if (workspaceReview && !isDispatchingAction) {
      const commit = (): void => {
        workspaceReviewState.setReviewTitle('');
        workspaceReviewState.commitWorkspaceReview(workspaceReview).catch(applicationStore.alertIllegalUnhandledError);
      };
  ...
  const createReview = (): void => {
    if (workspaceReviewState.reviewTitle && !workspaceReview && !isDispatchingAction) {
      workspaceReviewState.createWorkspaceReview(workspaceReviewState.reviewTitle).catch(applicationStore.alertIllegalUnhandledError);
    }
  };
  ...
  useEffect(() => { workspaceReviewState.fetchCurrentWorkspaceReview().catch(applicationStore.alertIllegalUnhandledError) }, [applicationStore, workspaceReviewState]);
    ...
    <div className="panel workspace-review">
      <div className="panel__header side-bar__header">
        <div className="panel__header__title workspace-review__header__title">
          <div className="panel__header__title__content side-bar__header__title__content">REVIEW</div>
        </div>
        <div className="panel__header__actions side-bar__header__actions">
          <button
            className={clsx('panel__header__action side-bar__header__action workspace-review__refresh-btn',
      ...
      <div className="panel__content side-bar__content">
        ...
        <div className="panel workspace-review">
          {!workspaceReview &&
            <form className="workspace-review__title" onSubmit={(e): void => { e.preventDefault() }}>
              ...
              <button
                ...
                disabled={isDispatchingAction || Boolean(workspaceReview) || !workspaceReviewState.reviewTitle}
                title={'Create review'}
              ><FaPlus /></button>
            </form>
          }
          {workspaceReview &&
            <>
              <div className="workspace-review__title">
                <div className="workspace-review__title__content">
                  <div className="workspace-review__title__content__input workspace-review__title__content__input--with-link" title={'See review detail'}>
                    <Link className="workspace-review__title__content__input__link" rel="noopener noreferrer" target="_blank" to={getReviewRoute(workspaceReview.projectId, workspaceReview.id)}>
                      <span className="workspace-review__title__content__input__link__review-name">{workspaceReview.title}</span>
                      <div className="workspace-review__title__content__input__link__btn"><FaExternalLinkSquareAlt /></div>
                    </Link>
                  </div>
                </div>
                <button
              ...
              <div className="workspace-review__title__content__review-status">
                created {formatDistanceToNow(workspaceReview.createdAt, { includeSeconds: true, addSuffix: true })}
              </div>
            </>
          }
