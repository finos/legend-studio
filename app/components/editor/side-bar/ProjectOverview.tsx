...
import { VERSION_TYPE } from 'SDLC/version/CreateVersionCommand';
...
import { getEditorRoute, getVersionViewerRoute, getReviewRoute } from 'Stores/RouterConfig';
...
const WorkspaceViewerContextMenu = observer<{
  ...
  const deleteWorkspace = applicationStore.guaranteeSafeAction(() => editorStore.projectOverviewState.deleteWorkspace(workspace.workspaceId));

  return (
    <div ref={ref} className="project-overview__context-menu">
      <div className="project-overview__context-menu__item" onClick={deleteWorkspace}>Delete</div>
    </div>
  );
}, { forwardRef: true });
...
const WorkspaceViewer = observer((props: {
  ...
  const { workspace } = props;
  const editorStore = useEditorStore();
  const isActive = editorStore.sdlcState.currentWorkspaceId === workspace.workspaceId;
    ...
    >
      ...
      >
        <div className="project-overview__item__link__content">
          <span className="project-overview__item__link__content__name">{workspace.workspaceId}</span>
        </div>
      </Link>
    </ContextMenu>
  );
...
const WorkspacesViewer = observer(() => {
  ...
  const isDispatchingAction = projectOverviewState.isDeletingWorkspace || projectOverviewState.isFetchingProjectWorkspaces;

  // since this can be affected by other users, we refresh it more proactively
  useEffect(() => { projectOverviewState.fetchProjectWorkspaces().catch(applicationStore.alertIllegalUnhandledError) }, [applicationStore, projectOverviewState]);

  return (
    <div className="panel side-bar__panel project-overview__panel project-overview__workspaces">
      <div className="panel__header">
        <div className="panel__header__title">
          <div className="panel__header__title__content">{PROJECT_OVERVIEW_ACTIVITY_MODE.WORKSPACES}</div>
        </div>
        <div className="side-bar__panel__header__changes-count" data-testid={TEST_ID.SIDEBAR_PANEL_HEADER__CHANGES_COUNT}>{workspaces.length}</div>
      </div>
      <div className="panel__content project-overview__panel__content">
        <PanelLoadingIndicator isLoading={isDispatchingAction} />
        <div data-testid={TEST_ID.PANEL_CONTENT_LIST} className="panel__content__list">
          {workspaces.map(workspace => <WorkspaceViewer key={workspace.workspaceId} workspace={workspace} />)}
        </div>
      </div>
    </div>
  );
...
const ReleaseEditor = observer(() => {
  ...
  const createMajorRelease = applicationStore.guaranteeSafeAction(() => projectOverviewState.createVersion(VERSION_TYPE.MAJOR));
  const createMinorRelease = applicationStore.guaranteeSafeAction(() => projectOverviewState.createVersion(VERSION_TYPE.MINOR));
  const createPatchRelease = applicationStore.guaranteeSafeAction(() => projectOverviewState.createVersion(VERSION_TYPE.PATCH));
  const changeNotes: React.ChangeEventHandler<HTMLTextAreaElement> = event => revisionInput.setNotes(event.target.value);
  const isCurrentProjectVersionLatest = Boolean(latestProjectVersion) && latestProjectVersion?.revisionId === currentProjectRevision?.id;
  const notFetchedLatestVersionAndCurrentRevision = latestProjectVersion === undefined || currentProjectRevision === undefined;

  // since this can be affected by other users, we refresh it more proactively
  useEffect(() => { projectOverviewState.fetchLatestProjectVersion().catch(applicationStore.alertIllegalUnhandledError) }, [applicationStore, projectOverviewState]);

  if (!sdlcState.isCurrentProjectInProduction) {
    return <div className="panel__content project-overview__release--empty">Release is only supported for PROD projects</div>;
  }
  return (
    <div className="panel side-bar__panel project-overview__panel project-overview__release">
      <div className="panel__header">
        <div className="panel__header__title">
          <div className="panel__header__title__content">{PROJECT_OVERVIEW_ACTIVITY_MODE.RELEASE}</div>
        </div>
      </div>
        ...
        <div className="project-overview__release__editor">
          ...
          <div className="project-overview__release__editor__actions">
            <button
              ...
              disabled={isCurrentProjectVersionLatest || isDispatchingAction}
              title={'Create a major release which comes with backward-incompatible features'}
            >MAJOR</button>
            <button
              className="project-overview__release__editor__action btn--dark"
              onClick={createMinorRelease}
              disabled={isCurrentProjectVersionLatest || isDispatchingAction}
              title={'Create a minor release which comes with backward-compatible features'}
            >MINOR</button>
            <button
              className="project-overview__release__editor__action btn--dark"
              onClick={createPatchRelease}
              disabled={isCurrentProjectVersionLatest || isDispatchingAction}
              title={'Create a patch release which comes with backward-compatible bug fixes'}
            >PATCH</button>
          </div>
        </div>
        {!notFetchedLatestVersionAndCurrentRevision &&
          <div className="project-overview__release__info">
            <div className="panel project-overview__release__info__current-version__container">
              <div className="panel__header">
                <div className="panel__header__title">
                  <div className="panel__header__title__content">LATEST RELEASE</div>
                </div>
              </div>
              <div className="panel__content">
                {latestProjectVersion &&
                  <div className="project-overview__release__info__current-version">
                    <Link className="project-overview__release__info__current-version__link" rel="noopener noreferrer" target="_blank" to={getVersionViewerRoute(latestProjectVersion.projectId, latestProjectVersion.id.id)}>
                      <div className="project-overview__release__info__current-version__link__content">
                        <span className="project-overview__release__info__current-version__link__content__name">{latestProjectVersion.id.id}</span>
                        <span className="project-overview__release__info__current-version__link__content__info">{latestProjectVersion.notes}</span>
                      </div>
                    </Link>
                  </div>
                }
                {!latestProjectVersion &&
                  <div className="project-overview__release__info__current-version">
                    <span className="project-overview__release__info__current-version__no-version">This project has no release</span>
                  </div>
                }
              </div>
            </div>
            <div className="panel project-overview__release__info__reviews">
              <div className="panel__header">
                <div className="panel__header__title">
                  <div className="panel__header__title__content">COMMITTED REVIEWS</div>
                  <div className="side-bar__panel__title__info" title="All committed reviews in the project since the latest release">
                    <FaInfoCircle />
                  </div>
                </div>
                <div className="side-bar__panel__header__changes-count" data-testid={TEST_ID.SIDEBAR_PANEL_HEADER__CHANGES_COUNT}>{commitedReviews.length}</div>
              </div>
              <div className="panel__content">
                {commitedReviews.map(review =>
                  ...
                  >
                    <div className="workspace-updater__review">
                      <span className="workspace-updater__review__name">{review.title}</span>
                      <span className="workspace-updater__review__info">{review.author.name}</span>
                    </div>
                  </Link>
                )}
              </div>
...
const VersionsViewer = observer(() => {
  ...
  // since this can be affected by other users, we refresh it more proactively
  useEffect(() => { editorStore.sdlcState.fetchProjectVersions().catch(applicationStore.alertIllegalUnhandledError) }, [applicationStore, editorStore]);

  return (
    <div className="panel side-bar__panel project-overview__panel project-overview__versions">
      <div className="panel__header">
        <div className="panel__header__title">
          <div className="panel__header__title__content">{PROJECT_OVERVIEW_ACTIVITY_MODE.VERSIONS}</div>
        </div>
        <div className="side-bar__panel__header__changes-count" data-testid={TEST_ID.SIDEBAR_PANEL_HEADER__CHANGES_COUNT}>{versions.length}</div>
      </div>
      <div className="panel__content project-overview__panel__content">
        <PanelLoadingIndicator isLoading={isDispatchingAction} />
        <div className="panel__content__list">
          {versions.map(version =>
            ...
            >
              <div className="project-overview__item__link__content">
                <span className="project-overview__item__link__content__name">{version.id.id}</span>
                <span className="project-overview__item__link__content__info">{version.notes}</span>
              </div>
            </Link>
          )}
        </div>
...
const OverviewViewer = observer(() => {
  ...
  const handleUpdate = (event: React.MouseEvent<HTMLButtonElement, MouseEvent>): void => {
    event.preventDefault();
    projectOverviewState.updateProject(projectIdentifier, description, tagsArray).catch(applicationStore.alertIllegalUnhandledError);
  };
  ...
  return (
    <div className="panel side-bar__panel project-overview__panel project-overview__overview">
      <div className="panel__header">
        <div className="panel__header__title">
          <div className="panel__header__title__content">{PROJECT_OVERVIEW_ACTIVITY_MODE.OVERVIEW}</div>
        </div>
        <button
      ...
      <div className="panel__content project-overview__panel__content">
        ...
        <div className="panel__content__form">
          <div className="panel__content__form__section">
            <div className="panel__content__form__section__header__label">Name</div>
...
export const ProjectOverviewActivityBar = observer(() => {
  const editorStore = useEditorStore();
  const projectOverviewState = editorStore.projectOverviewState;
  const changeActivity = (activity: PROJECT_OVERVIEW_ACTIVITY_MODE): () => void => (): void => projectOverviewState.setActivityMode(activity);
  const activities: ProjectOverviewActivityDisplay[] = [
    { mode: PROJECT_OVERVIEW_ACTIVITY_MODE.OVERVIEW, title: 'Overview' },
    editorStore.sdlcState.isCurrentProjectInProduction && { mode: PROJECT_OVERVIEW_ACTIVITY_MODE.RELEASE, title: 'Release' },
  ...
  ].filter((activity): activity is ProjectOverviewActivityDisplay => Boolean(activity));

  return (
    <div data-testid={TEST_ID.PROJECT_OVERVIEW__ACTIVITY_BAR} className="project-overview__activity-bar" >
      <div className="project-overview__activity-bar__items">
        {activities.map(activity =>
          <div
            key={activity.mode} className={clsx('project-overview__activity-bar__item', { 'project-overview__activity-bar__item--active': activity.mode === projectOverviewState.activityMode })}
            onClick={changeActivity(activity.mode)}
            tabIndex={-1}
            title={activity.title}
          ><div className="project-overview__activity-bar__item-mode">{activity.mode}</div></div>
        )}
      </div>
    </div >
  );
...
export const ProjectOverview = observer(() => {
  ...
  const renderOverview = (): React.ReactNode => {
    switch (projectOverviewState.activityMode) {
      case PROJECT_OVERVIEW_ACTIVITY_MODE.OVERVIEW: return <OverviewViewer />;
      case PROJECT_OVERVIEW_ACTIVITY_MODE.RELEASE: return <ReleaseEditor />;
      case PROJECT_OVERVIEW_ACTIVITY_MODE.VERSIONS: return <VersionsViewer />;
      case PROJECT_OVERVIEW_ACTIVITY_MODE.WORKSPACES: return <WorkspacesViewer />;
      default: return null;
    }
  };
  ...
  useEffect(() => {
    if (projectOverviewState.activityMode === PROJECT_OVERVIEW_ACTIVITY_MODE.RELEASE && !editorStore.sdlcState.isCurrentProjectInProduction) {
      projectOverviewState.setActivityMode(PROJECT_OVERVIEW_ACTIVITY_MODE.OVERVIEW);
    }
  }, [projectOverviewState.activityMode, editorStore.sdlcState.isCurrentProjectInProduction, projectOverviewState]);

  return (
    <div className="panel project-overview">
      <div className="panel__header side-bar__header">
        <div className="panel__header__title">
          <div className="panel__header__title__content side-bar__header__title__content">PROJECT</div>
        </div>
      </div>
