...
import { getProjectViewerRoute, getVersionViewerRoute } from 'Stores/RouterConfig';
...
const ShareModal = observer((props: {
  ...
  const projectLink = selectedVersion
    ? `${urlBase}${applicationStore.historyApiClient.createHref({ pathname: getVersionViewerRoute(projectId, selectedVersion.id.id) })}`
    : `${urlBase}${applicationStore.historyApiClient.createHref({ pathname: getProjectViewerRoute(projectId) })}`;
  const copyProjectLink = (): void => {
    applicationStore.copyTextToClipboard(projectLink)
      .then(() => applicationStore.notifySuccess('Copied project link to clipboard'))
      .catch(applicationStore.alertIllegalUnhandledError)
      .finally(() => closeModal());
  };
  const renderOptions = versions.map(version => ({ label: version.id.id, value: version }));
  const onSelectionChange = (val: { label: string, value: Version } | null): void => setSelectedVersion(val?.value);
      ...
      <div className="modal modal--dark modal--no-padding">
        ...
        <div className="modal__body">
          <div className="share-project__modal__info-entry">
            <div className="share-project__modal__info-entry__title">Version:</div>
                ...
                ? (
                  <div className="share-project__modal__select">
                    <CustomSelectorInput
                      ...
                      value={selectedVersion ? { label: selectedVersion.id.id, value: selectedVersion } : null}
                      darkMode={true}
                    />
                  </div>
                )
          ...
          <div className="share-project__modal__info-entry">
            <div className="share-project__modal__info-entry__title">Link:</div>
            <div className="share-project__modal__info-entry__value">
        ...
        <div className="modal__footer">
          <button className="btn--wide btn--dark" disabled={isFetchingProject} onClick={copyProjectLink}>Copy Link</button>
        </div>
      </div>
    </Dialog>
  );
...
export const ShareProjectHeaderAction = observer(() => {
  ...
  return (
    <div className="app__header__action">
      ...
      ><FiShare /></button>
      {editorStore.sdlcState.currentProject && <ShareModal open={openShareModal} closeModal={hideShareModal} />}
    </div>
  );
});
