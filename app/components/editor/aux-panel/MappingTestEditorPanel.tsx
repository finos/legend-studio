...
import { MappingTestState, MappingTestGraphFetchTreeQueryState, MappingTestObjectInputDataState, MappingTestExpectedOutputAssertionState } from 'Stores/editor-state/element-editor-state/mapping/MappingTestState';
import clsx from 'clsx';
import Dialog from '@material-ui/core/Dialog';
import { EDITOR_LANGUAGE } from 'Stores/EditorConfig';
import { isNonNullable, assertType, guaranteeType, compareLabelFn } from 'Utilities/GeneralUtil';
...
import { ErrorIcon } from 'Components/shared/Icon';
import { getMappingElementSourceSelectOption } from 'Components/editor/aux-panel/MappingExecution';
...
import { getMappingElementSource, getMappingElementTarget, getMappingElementSourceFilterText, MappingElementSource, MappingElementSourceSelectOption } from 'MM/model/packageableElements/mapping/Mapping';
import { createValidationError } from 'MM/validator/ValidationResult';

export const MappingTestGraphFetchQueryBuilder = observer((props: {
  ...
  const validationResult = testState.test.validationResult ??
    ...
    editorStore.graphState.graphManager.HACKY_isGetAllLambda(testState.test.query)
  ...
  const targetOptions = testState.mappingEditorState.mapping.getAllMappingElements()
  ...
  const changeTarget = (val: Class | undefined): void => {
    if (val) {
      queryState.setTarget(val);
      queryState.setGraphFetchTree(getGraphFetchTreeData(val));
      ...
      const possibleSources = mappingEditorState.mapping.getAllMappingElements()
        .filter(mappingElement => getMappingElementTarget(mappingElement) === val)
        .map(mappingElement => getMappingElementSource(mappingElement))
        .filter(isNonNullable);
      testState.setInputDataStateBasedOnSource(possibleSources.length ? possibleSources[0] : undefined, true);
    } else {
  ...
  // Drag and Drop
  const handleDrop = (item: MappingExecutionTargetDropTarget): void => isReadOnly ? undefined : changeTarget(guaranteeType(getMappingElementTarget(item.data), Class));
    ...
    <div className="panel mapping-test-editor-panel__target-panel">
      <div className="panel__header">
        <div className="panel__header__title">
          ...
          <div className="panel__header__title__content">{target?.name ?? '(none)'}</div>
        </div>
      ...
      <div ref={dropRef} className={clsx('panel__content', { 'panel__content--dnd-over': isDragOver && !isReadOnly }, { 'panel__content--has-validation-error': !isValid })}>
        {!isValid && <div className="panel__content__validation-error" title={validationResult?.messages.join('\n') ?? ''}><ErrorIcon /></div>}
        {graphFetchTree &&
          <div className="mapping-test-editor-panel__target-panel__query-container">
            <GraphFetchTreeExplorer treeData={graphFetchTree} updateTreeData={updateTreeData} isReadOnly={isReadOnly} parentMapping={mappingEditorState.mapping} />
          </div>
        }
        {!graphFetchTree &&
          <div className="mapping-test-editor-panel__target" onClick={showTargetSelectorModal}>
            <div className="mapping-test-editor-panel__target__text">Choose a target</div>
            <div className="mapping-test-editor-panel__target__action"><MdVerticalAlignBottom /></div>
          </div>
        }
      </div>
      <Dialog
...
export const MappingTestQueryBuilder = observer((props: {
  mappingEditorState: MappingEditorState;
  testState: MappingTestState;
}) => {
  ...
  if (queryState instanceof MappingTestGraphFetchTreeQueryState) {
    return <MappingTestGraphFetchQueryBuilder mappingEditorState={mappingEditorState} testState={testState} queryState={queryState} />;
  }
  return null;
});
    ...
    <div className="panel mapping-test-editor-panel__source-panel">
      <div className="panel__header">
        <div className="panel__header__title">
          ...
          <div className="panel__header__title__content">{inputDataState.sourceClass?.name ?? '(none)'}</div>
        </div>
        <div className="panel__header__actions">
          <button
            className="panel__header__action"
      ...
      <div className={clsx('panel__content mapping-test-editor-panel__text-editor', { 'panel__content--has-validation-error': !isValid })}>
        {!isValid && <div className="panel__content__validation-error" title={validationResult?.messages.join('\n') ?? ''}><ErrorIcon /></div>}
        <TextInputEditor inputValue={inputDataState.data} updateInput={updateInputData} isReadOnly={isReadOnly} language={EDITOR_LANGUAGE.JSON} />
      </div>
      <Dialog
...
export const MappingTestInputDataBuilder = observer((props: {
  testState: MappingTestState;
  mappingEditorState: MappingEditorState;
}) => {
  const { mappingEditorState, testState } = props;
  ...
  if (inputDataState instanceof MappingTestObjectInputDataState) {
    assertType(queryState, MappingTestGraphFetchTreeQueryState, 'Model-to-model mapping test only support graph fetch tree query type');
    return <MappingTestObjectInputDataBuilder mappingEditorState={mappingEditorState} testState={testState} queryState={queryState} inputDataState={inputDataState} />;
  }
  return null;
});
...
export const MappingTestExpectedOutputAssertionBuilder = observer((props: {
  ...
  const formatExpectedResultJSONString = (): void => assertionState.setExpectedResult(tryToFormatJSONString(assertionState.expectedResult));
  // Actions
  const regenerateExpectedResult = applicationStore.guaranteeSafeAction(() => testState.regenerateExpectedResult());
  ...
  return (
    <div className="panel mapping-test-editor-panel__expected-result-panel">
      <div className="panel__header">
        ...
        <div className="panel__header__actions">
          ...
          <button
            ...
            tabIndex={-1}
            title={'Regenerate expected execution result'}
          ><MdRefresh /></button>
          <button
            className="panel__header__action"
            disabled={isReadOnly}
      ...
      <div className={clsx('panel__content mapping-test-editor-panel__text-editor', { 'panel__content--has-validation-error': !isValid })}>
        {!isValid && <div className="panel__content__validation-error" title={validationResult?.messages.join('\n') ?? ''}><ErrorIcon /></div>}
        <TextInputEditor inputValue={assertionState.expectedResult} updateInput={updateExpectedResult} isReadOnly={isReadOnly} language={EDITOR_LANGUAGE.JSON} />
      </div>
    </div>
  );
});

export const MappingTestAssertionBuilder = observer((props: {
  testState: MappingTestState;
  mappingEditorState: MappingEditorState;
}) => {
  const { mappingEditorState, testState } = props;
  ...
  if (assertionState instanceof MappingTestExpectedOutputAssertionState) {
    return <MappingTestExpectedOutputAssertionBuilder mappingEditorState={mappingEditorState} testState={testState} assertionState={assertionState} />;
  }
  return null;
});
...
export const MappingExecutionBuilder = observer((props: {
  ...
  useEffect(() => { testState.openTest(false).catch(applicationStore.alertIllegalUnhandledError) }, [applicationStore, testState]);
    ...
    <div className="mapping-test-editor-panel">
      ...
      <SplitPane split="vertical" defaultSize={500} minSize={500} maxSize={-250}>
        <SplitPane split="vertical" defaultSize={250} minSize={250} maxSize={-250}>
          ...
          <MappingTestQueryBuilder key={testState.queryState.uuid} mappingEditorState={mappingEditorState} testState={testState} />
          <MappingTestInputDataBuilder key={testState.inputDataState.uuid} mappingEditorState={mappingEditorState} testState={testState} />
        </SplitPane>
        <MappingTestAssertionBuilder key={testState.assertionState.uuid} mappingEditorState={mappingEditorState} testState={testState} />
      </SplitPane>
    </div>
  );
});
...
export const MappingTestEditorPanel = observer(() => {
  ...
  if (!currentElementState || !(currentElementState instanceof MappingEditorState) || !(currentElementState.currentTabState instanceof MappingTestState)) {
    ...
    return <div className="mapping-test-editor-panel mapping-test-editor-panel--no-content">Select a test to see detail</div>;
  }
  // NOTE: using the key here is crucial as it helps resetting component state properly as we change the mapping test state
  return <MappingExecutionBuilder key={currentElementState.currentTabState.uuid} testState={currentElementState.currentTabState} mappingEditorState={currentElementState} />;
});
