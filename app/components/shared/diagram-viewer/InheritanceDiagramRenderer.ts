...
export class InheritanceDiagramRenderer extends DiagramRenderer {
  ...
  loadClass(_class: Class): void {
    this.diagram = this.compute(this.buildLevels([new ClassView(this.diagram, '', PackageableElementExplicitReference.create(_class))]));
  }

  // TODO?: build sub types as well
  buildLevels(classViews: ClassView[]): ClassView[][] {
    if (classViews.length) {
      classViews.forEach(classView => this.computeClassViewMinDimensions(classView));
      const res = classViews.flatMap(classView => classView.class.value.generalizations.map(generation =>
        new ClassView(this.diagram, '', PackageableElementExplicitReference.create(generation.value.getRawType(Class)))));
  ...
  compute(classViewLevels: ClassView[][]): Diagram {
    ...
    const classViews = classViewLevels.flatMap((level, i) => {
      ...
      const maxHeight = i === 0 ? 0 : Math.max(...classViewLevels[i - 1].map(classView => classView.rectangle.height));
      const totalWidth = level.map(classView => classView.rectangle.width).reduce((a, b) => a + b);
      ...
      return level.flatMap((view, index) => {
        if (index) {
          ...
          view.setPosition(new Point(precedent.position.x + precedent.rectangle.width, view.position.y));
        }
    ...
    const generalizationViews = classViewLevels.flatMap((level, i) => {
      if (i > 0) {
        return level.flatMap(fromClassView => classViewLevels[i - 1].flatMap(toClassView => {
          if (fromClassView.class.value.generalizations.map(g => g.value.rawType).includes(toClassView.class.value)) {
            return new GeneralizationView(newDiagram, fromClassView, toClassView);
          }
          return [];
        }));
      }
