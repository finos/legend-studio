...
import { uuid, noop, getNullableFirstElement } from 'Utilities/GeneralUtil';
import { PositionedRectangle } from 'MM/model/packageableElements/diagram/geometry/PositionedRectangle';
import { Point } from 'MM/model/packageableElements/diagram/geometry/Point';
import { Rectangle } from 'MM/model/packageableElements/diagram/geometry/Rectangle';
...
import { GeneralizationView } from 'MM/model/packageableElements/diagram/GeneralizationView';
import { RelationshipView, manageInsidePointsDynamically, getElementPosition } from 'MM/model/packageableElements/diagram/RelationshipView';
import { PropertyView } from 'MM/model/packageableElements/diagram/PropertyView';
import { PropertyHolderView } from 'MM/model/packageableElements/diagram/PropertyHolderView';
import { AssociationView } from 'MM/model/packageableElements/diagram/AssociationView';
...
export class DiagramRenderer {
   ...
   * As such, when we debug, let's say we have a position (x,y), if we need to find that coordinate in the coordiante system of the canvas, we have to
  ...
  mousedOverProperty?: AbstractProperty;
  mousedOverClassView?: ClassView;
  ...
  constructor(div: HTMLDivElement, diagram: Diagram) {
    ...
    this.diamond = [new Point(0, 0), new Point(-10, -5), new Point(-20, 0), new Point(-10, 5)];

    // Font
    // this.fontFamily = 'Source Code Pro'; // intentionally choose a monospaced font so it's easier for calculation (such as text wrapping)
  ...
  start(): void {
    this.diagram.classViews.forEach(classView => this.computeClassViewMinDimensions(classView));
    this.refresh();
  }
  ...
  autoRecenter(): void {
    this.recenter(this.virtualScreen.position.x + this.virtualScreen.rectangle.width / 2, this.virtualScreen.position.y + this.virtualScreen.rectangle.height / 2);
  }
  ...
  truncateTextWithEllipsis(val: string, limit = this.maxLineLength): string {
    const ellipsis = '...';
    return val.length > limit ? `${val.substring(0, limit + 1 - ellipsis.length)}${ellipsis}` : val;
  }
  ...
  toModelCoordinate(point: Point): Point {
    return new Point(
      (point.x - this.canvasCenter.x) / this.zoom - this.screenOffset.x + this.canvasCenter.x,
      (point.y - this.canvasCenter.y) / this.zoom - this.screenOffset.y + this.canvasCenter.y,
    );
  }
  ...
  toCanvasCoordinate(point: Point): Point {
    return new Point(
      (point.x - this.canvasCenter.x + this.screenOffset.x) * this.zoom + this.canvasCenter.x,
      (point.y - this.canvasCenter.y + this.screenOffset.y) * this.zoom + this.canvasCenter.y,
    );
  }
  ...
  manageVirtualScreen(): void {
    if (this.diagram.classViews.length) {
      ...
      let maxX = this.diagram.classViews[0].position.x + this.diagram.classViews[0].rectangle.width;
      let maxY = this.diagram.classViews[0].position.y + this.diagram.classViews[0].rectangle.height;
      ...
      const relationshipViews = (this.diagram.associationViews as RelationshipView[]).concat(this.diagram.generalizationViews).concat(this.diagram.propertyViews);
      for (const relationshipView of relationshipViews) {
        ...
        if (relationshipView instanceof PropertyView) {
          const box = this.drawLineText(fullPath[fullPath.length - 2], fullPath[fullPath.length - 1], relationshipView.to.classView.value, relationshipView.property.value);
   ...
   * This function is intended to be used with drag and drop, hence the position parameter, which must be relative to the screen/window
   */
  addClassView(addedClass: Class, absolutePosition?: Point): ClassView | undefined {
    if (!this.isReadOnly) {
      ...
      newClassView.setPosition(this.toModelCoordinate(new Point(
      ...
      this.diagram.classViews
        ...
        .forEach(classView => {
          ...
          // Add supertype
          if (addedClass.generalizations.map(generalization => generalization.value.rawType).includes(_class)) {
            this.diagram.addGeneralizationView(new GeneralizationView(this.diagram, newClassView, classView));
          }
          if (_class.generalizations.map(generalization => generalization.value.rawType).includes(addedClass)) {
  ...
  drawAll(): void {
    ...
    this.ctx.fillRect(
      (this.virtualScreen.position.x + this.screenOffset.x - this.canvasCenter.x - this.screenPadding) * this.zoom + this.canvasCenter.x,
      (this.virtualScreen.position.y + this.screenOffset.y - this.canvasCenter.y - this.screenPadding) * this.zoom + this.canvasCenter.y,
      (this.virtualScreen.rectangle.width + this.screenPadding * 2) * this.zoom,
      (this.virtualScreen.rectangle.height + this.screenPadding * 2) * this.zoom,
    );
    this.ctx.strokeRect(
      (this.virtualScreen.position.x + this.screenOffset.x - this.canvasCenter.x - this.screenPadding) * this.zoom + this.canvasCenter.x,
      (this.virtualScreen.position.y + this.screenOffset.y - this.canvasCenter.y - this.screenPadding) * this.zoom + this.canvasCenter.y,
    ...
    if (this.showCanvasGrid) { this.drawCanvasGrid() }
    if (this.showScreenGrid) { this.drawScreenGrid() }
  }

  drawScreenGrid(): void {
    const startX = (this.virtualScreen.position.x + this.screenOffset.x - this.canvasCenter.x) * this.zoom + this.canvasCenter.x;
    const startY = (this.virtualScreen.position.y + this.screenOffset.y - this.canvasCenter.y) * this.zoom + this.canvasCenter.y;
    const width = (this.virtualScreen.rectangle.width + this.screenPadding * 2) * this.zoom;
    const height = (this.virtualScreen.rectangle.height + this.screenPadding * 2) * this.zoom;
    this.ctx.beginPath();
    ...
    for (let x = startX; x < startX + width - this.screenPadding * 2; x += this.screenGridSpacingX * this.zoom) {
      this.ctx.fillText(`${gridX}`, x + labelPadding * this.zoom, startY - (this.screenGridAxisTickLength + this.screenPadding) * this.zoom);
      this.ctx.fillText(`[${Math.round(this.virtualScreen.position.x + gridX)}]`, x + labelPadding * this.zoom, startY - (this.screenGridAxisTickLength + this.screenPadding - this.lineHeight) * this.zoom);
      this.ctx.moveTo(x, startY - (this.screenGridAxisTickLength + this.screenPadding) * this.zoom);
    ...
    for (let y = startY; y < startY + height - this.screenPadding * 2; y += this.screenGridSpacingY * this.zoom) {
      this.ctx.fillText(`${gridY}`, startX - (this.screenGridAxisTickLength + this.screenPadding) * this.zoom, y + labelPadding * this.zoom);
      this.ctx.fillText(`[${Math.round(this.virtualScreen.position.y + gridY)}]`, startX - (this.screenGridAxisTickLength + this.screenPadding) * this.zoom, y + (labelPadding + this.lineHeight) * this.zoom);
      this.ctx.moveTo(startX - (this.screenGridAxisTickLength + this.screenPadding) * this.zoom, y);
    ...
    if (this.showScreenBoxGuide) {
      ...
      this.ctx.fillText(`[${Math.round(this.virtualScreen.position.x + this.virtualScreen.rectangle.width / 2)}]`, startX + width / 2 - (this.screenPadding - labelPadding) * this.zoom, startY + (labelPadding + this.lineHeight) * this.zoom);
      ...
      this.ctx.fillText(`[${Math.round(this.virtualScreen.position.y + this.virtualScreen.rectangle.height / 2)}]`, startX + labelPadding * this.zoom, startY + height / 2 - (this.screenPadding - labelPadding - this.lineHeight) * this.zoom);
  ...
  drawCanvasGrid(): void {
    ...
    for (let x = startX; x < width; x += this.canvasGridSpacingX * this.zoom) {
      if (x !== 0) { this.ctx.fillText(`[${Math.round(gridX)}]`, x + labelPadding * this.zoom, labelPadding * this.zoom) }
      this.ctx.moveTo(x, 0);
    ...
    for (let y = startY; y < height; y += this.canvasGridSpacingY * this.zoom) {
      if (y !== 0) { this.ctx.fillText(`[${Math.round(gridY)}]`, labelPadding * this.zoom, y + labelPadding * this.zoom) }
      this.ctx.moveTo(0, y);
    ...
    if (this.showScreenOffsetGuide) {
      ...
      this.ctx.stroke();
      this.ctx.beginPath();
      if (this.screenOffset.x >= thresholdToHideOffsetTickLabel && this.screenOffset.y >= thresholdToHideOffsetTickLabel) {
        ...
        this.ctx.fillText(`\u0394 [${Math.round(this.screenOffset.x)},${Math.round(this.screenOffset.y)}]`,
          startX + (this.screenOffset.x + offsetPointRadius + labelPadding) * this.zoom,
          startY + (this.screenOffset.y + offsetPointRadius + labelPadding) * this.zoom
        );
      } else {
        this.ctx.fillText(`\u0394 [${Math.round(this.screenOffset.x)},${Math.round(this.screenOffset.y)}]`,
  ...
  drawProperty(classView: ClassView, property: AbstractProperty, measureOnly: boolean, propX: number, propY: number): number {
    this.ctx.font = this.mousedOverClassView === classView && this.mousedOverProperty === property
    ...
    if (!measureOnly) {
      ...
      if (property.genericType.value.rawType instanceof Enumeration) {
        ...
        this.ctx.lineTo(propX + txtMeasure + this.ctx.measureText(property.genericType.value.rawType.name).width, propY + (this.fontSize - 1) * this.zoom);
        this.ctx.stroke();
      }
    }
  ...
  computeClassViewMinDimensions(classView: ClassView): number {
    ...
    if (!classView.hideProperties) {
      classView.class.value.getAllOwnedProperties().forEach(property => {
        if (!this.hasPropertyView(classView, property)) {
          const txtMeasure = this.drawProperty(classView, property, true, -1, -1);
    ...
    if (!this.isReadOnly) {
      const width = classView.rectangle.width && classView.rectangle.width > classMinWidth ? classView.rectangle.width : classMinWidth;
      const height = classView.rectangle.height && classView.rectangle.height > classMinHeight ? classView.rectangle.height : classMinHeight;
  ...
  drawClassView(classView: ClassView): void {
    ...
    this.ctx.lineWidth = this.selectedClasses.length !== 0 && this.selectedClasses.indexOf(classView) !== -1 ? 2 : 1;
    ...
    if (!classView.hideStereotypes) {
      for (const stereotype of classView.class.value.stereotypes) {
        ...
        const stereoWidth = this.ctx.measureText(stereotypeTxt).width;
        this.ctx.font = `${this.fontSize * this.zoom}px ${this.fontFamily}`;
        this.ctx.fillText(stereotypeTxt, (startX + this.screenOffset.x + (classView.rectangle.width - stereoWidth) / 2 - this.canvasCenter.x) * this.zoom + this.canvasCenter.x, (cursorY + this.screenOffset.y + this.classViewSpaceY - this.canvasCenter.y) * this.zoom + this.canvasCenter.y);
    ...
    const classNameText = this.truncateTextWithEllipsis(classView.class.value.name);
    this.ctx.fillText(classNameText, (startX + this.screenOffset.x + (classView.rectangle.width - classMinWidth) / 2 - this.canvasCenter.x) * this.zoom + this.canvasCenter.x, (cursorY + this.screenOffset.y + this.classViewSpaceY - this.canvasCenter.y) * this.zoom + this.canvasCenter.y);
    cursorY = cursorY + this.lineHeight;
    ...
    if (!classView.hideProperties) {
      ...
      this.ctx.beginPath();
      const yLineCoord = (startY + this.screenOffset.y + this.lineHeight
        + (!classView.hideStereotypes ? classView.class.value.stereotypes.length * this.lineHeight : 0)
        + (!classView.hideTaggedValues ? classView.class.value.taggedValues.length * this.lineHeight : 0)
        + this.classViewSpaceY
        - this.canvasCenter.y) * this.zoom + this.canvasCenter.y;
      this.ctx.moveTo((startX + this.screenOffset.x - this.canvasCenter.x) * this.zoom + this.canvasCenter.x, yLineCoord);
      this.ctx.lineTo((startX + this.screenOffset.x + classView.rectangle.width - this.canvasCenter.x) * this.zoom + this.canvasCenter.x, yLineCoord);
      this.ctx.stroke();

      for (const property of classView.class.value.getAllOwnedProperties()) {
        if (!this.hasPropertyView(classView, property)) {
          this.ctx.fillStyle = property instanceof DerivedProperty
          ...
          const propX = (startX + this.screenOffset.x + this.classViewSpaceX - this.canvasCenter.x) * this.zoom + this.canvasCenter.x;
          const propY = (cursorY + this.screenOffset.y - this.canvasCenter.y) * this.zoom + this.canvasCenter.y;
  ...
  drawLinePropertyAndMultiplicityText(property: AbstractProperty, textPositionX: (n: number) => number, textPositionY: (n: number) => number, multiplicityPositionX: (n: number) => number, multiplicityPositionY: (n: number) => number): PositionedRectangle {
    this.ctx.font = `${this.fontSize}px ${this.fontFamily}`;
    const propertyName = this.propertyName(property);
    ...
    const propertyPosition = this.toCanvasCoordinate(new Point(textPositionX(textSize), textPositionY(textSize)));
    ...
    const multiplicityPosition = this.toCanvasCoordinate(new Point(multiplicityPositionX(mulSize), multiplicityPositionY(mulSize)));
    ...
    return new PositionedRectangle(
      position,
      new Rectangle((Math.max(posX, mulPosX) + Math.max(textSize, mulSize)) - position.x, (Math.max(posY, mulPosY) + this.lineHeight) - position.y)
    );
  }

  drawLineText(from: Point, to: Point, viewSide: ClassView, property: AbstractProperty): PositionedRectangle {
    ...
    if (startY < endY) {
      const x = startX + (endX - startX) / (endY - startY) * (viewSide.position.y - startY);
      if (x > viewSide.position.x && x < viewSide.position.x + rect.width) {
        return this.drawLinePropertyAndMultiplicityText(property,
          textSize => x < viewSide.position.x + rect.width / 2 ? x + this.propertySpacing : x - textSize - this.propertySpacing,
          textSize => viewSide.position.y - this.lineHeight - this.propertySpacing,
          mulSize => x < viewSide.position.x + rect.width / 2 ? x - mulSize - this.propertySpacing : x + this.propertySpacing,
          mulSize => viewSide.position.y - this.lineHeight - this.propertySpacing);
      }
    } else {
      const x = startX + (endX - startX) / (endY - startY) * (viewSide.position.y + rect.height - startY);
      if (x > viewSide.position.x && x < viewSide.position.x + rect.width) {
        return this.drawLinePropertyAndMultiplicityText(property,
          textSize => x < viewSide.position.x + rect.width / 2 ? x + this.propertySpacing : x - textSize - this.propertySpacing,
          textSize => viewSide.position.y + rect.height + this.propertySpacing,
          mulSize => x < viewSide.position.x + rect.width / 2 ? x - mulSize - this.propertySpacing : x + this.propertySpacing,
          mulSize => viewSide.position.y + rect.height + this.propertySpacing);
      }
    }
    if (startX < endX) {
      const y = startY + (endY - startY) / (endX - startX) * (viewSide.position.x - startX);
      if (y > viewSide.position.y && y < viewSide.position.y + rect.height) {
        return this.drawLinePropertyAndMultiplicityText(property,
          textSize => viewSide.position.x - textSize - this.propertySpacing,
          textSize => y < viewSide.position.y + viewSide.rectangle.height / 2 ? y + this.propertySpacing : y - this.lineHeight - this.propertySpacing,
          mulSize => viewSide.position.x - mulSize - this.propertySpacing,
          mulSize => y < viewSide.position.y + viewSide.rectangle.height / 2 ? y - this.lineHeight - this.propertySpacing : y + this.propertySpacing);
      }
    } else {
      const y = startY + (endY - startY) / (endX - startX) * (viewSide.position.x + viewSide.rectangle.width - startX);
      if (y > viewSide.position.y && y < viewSide.position.y + viewSide.rectangle.height) {
        return this.drawLinePropertyAndMultiplicityText(property,
          textSize => viewSide.position.x + viewSide.rectangle.width + this.propertySpacing,
          textSize => y < viewSide.position.y + viewSide.rectangle.height / 2 ? y + this.propertySpacing : y - this.lineHeight - this.propertySpacing,
          mulSize => viewSide.position.x + viewSide.rectangle.width + this.propertySpacing,
          mulSize => y < viewSide.position.y + viewSide.rectangle.height / 2 ? y - this.lineHeight - this.propertySpacing : y + this.propertySpacing);
      }
    }
  ...
  drawPropertyOrAssociation(propertyView: PropertyView): void {
    ...
    this.drawLineText(fullPath[fullPath.length - 2], fullPath[fullPath.length - 1], propertyView.to.classView.value, toProperty);
    // if (asso.association) {
    //   this.displayText(fullPath[1], fullPath[0], asso.from.classView, asso.association.properties[0], this.ctx);
    // }
    this.ctx.beginPath();
    this.ctx.lineWidth = propertyView === this.selectedPropertyOrAssociation ? 2 : 1;
  ...
  drawInheritance(inheritance: GeneralizationView): void {
    ...
    let resultY = 0;
    if (endY > startY) {
      const x = startX + (endX - startX) / (endY - startY) * (inheritance.to.classView.value.position.y - startY);
      if (x > inheritance.to.classView.value.position.x && x < inheritance.to.classView.value.position.x + rect.width) {
        resultX = (x - this.canvasCenter.x) * this.zoom + this.canvasCenter.x;
        resultY = (inheritance.to.classView.value.position.y - this.canvasCenter.y) * this.zoom + this.canvasCenter.y;
      }
    } else {
      const x = startX + (endX - startX) / (endY - startY) * (inheritance.to.classView.value.position.y + rect.height - startY);
      if (x > inheritance.to.classView.value.position.x && x < inheritance.to.classView.value.position.x + rect.width) {
        resultX = (x - this.canvasCenter.x) * this.zoom + this.canvasCenter.x;
        resultY = (inheritance.to.classView.value.position.y + rect.height - this.canvasCenter.y) * this.zoom + this.canvasCenter.y;
      }
    }
    if (endX > startX) {
      const y = startY + (endY - startY) / (endX - startX) * (inheritance.to.classView.value.position.x - startX);
      if (y > inheritance.to.classView.value.position.y && y < inheritance.to.classView.value.position.y + rect.height) {
        resultX = (inheritance.to.classView.value.position.x - this.canvasCenter.x) * this.zoom + this.canvasCenter.x;
        resultY = (y - this.canvasCenter.y) * this.zoom + this.canvasCenter.y;
      }
    } else {
      const y = startY + (endY - startY) / (endX - startX) * (inheritance.to.classView.value.position.x + rect.width - startX);
      if (y > inheritance.to.classView.value.position.y && y < inheritance.to.classView.value.position.y + rect.height) {
        resultX = (inheritance.to.classView.value.position.x + rect.width - this.canvasCenter.x) * this.zoom + this.canvasCenter.x;
        resultY = (y - this.canvasCenter.y) * this.zoom + this.canvasCenter.y;
      }
    }
    this.ctx.beginPath();
    ...
    angle = endX >= startX ? angle : angle + Math.PI;
    this.ctx.beginPath();
    this.ctx.moveTo(resultX + (this.screenOffset.x + this.triangle[0].rotateX(angle)) * this.zoom, resultY + (this.screenOffset.y + this.triangle[0].rotateY(angle)) * this.zoom);
    this.ctx.lineTo(resultX + (this.screenOffset.x + this.triangle[1].rotateX(angle)) * this.zoom, resultY + (this.screenOffset.y + this.triangle[1].rotateY(angle)) * this.zoom);
    this.ctx.lineTo(resultX + (this.screenOffset.x + this.triangle[2].rotateX(angle)) * this.zoom, resultY + (this.screenOffset.y + this.triangle[2].rotateY(angle)) * this.zoom);
    this.ctx.lineTo(resultX + (this.screenOffset.x + this.triangle[0].rotateX(angle)) * this.zoom, resultY + (this.screenOffset.y + this.triangle[0].rotateY(angle)) * this.zoom);
  ...
  keydown(e: KeyboardEvent): void {
    ...
    if (e.keyCode === 46) {
      if (!this.isReadOnly) {
        this.selectedClasses.forEach(classView => {
          ...
          this.diagram.setAssociationViews(this.diagram.associationViews.filter(associationView => !(associationView.from.classView.value === classView || associationView.to.classView.value === classView)));
          this.diagram.setGeneralizationViews(this.diagram.generalizationViews.filter(generalizationView => !(generalizationView.from.classView.value === classView || generalizationView.to.classView.value === classView)));
          this.diagram.setPropertyViews(this.diagram.propertyViews.filter(propertyView => !(propertyView.from.classView.value === classView || propertyView.to.classView.value === classView)));
        });
    ...
    if (e.keyCode === 67) {
      if (this.selectedClasses.length !== 0) {
        ...
        if (firstClass) {
          this.recenter(firstClass.position.x + firstClass.rectangle.width / 2, firstClass.position.y + firstClass.rectangle.height / 2);
        }
      } else {
        this.autoRecenter();
      }
    ...
    // Add type view
    if (e.keyCode === 65) {
      if (this.mousedOverProperty) {
        if (this.mousedOverProperty.genericType.value.rawType instanceof Class) {
          this.addClassView(this.mousedOverProperty.genericType.value.rawType, new Point(this.cursorPosition.x, this.cursorPosition.y));
        }
      }
    }
    ...
    if (e.keyCode === 80) {
      if (this.selectedClasses.length !== 0) {
        this.selectedClasses.forEach(classView => this.onAddClassPropertyForSelectedClass(classView));
      }
    }
  ...
  // DOC ???
  potentiallyShiftRelationships(assoViews: RelationshipView[], selectedClasses: ClassView[], newMovingDeltaX: number, newMovingDeltaY: number): void {
    assoViews.forEach(assoView => {
      if (selectedClasses.indexOf(assoView.from.classView.value) !== -1 && selectedClasses.indexOf(assoView.to.classView.value) !== -1) {
  ...
  mousedblclick(e: MouseEvent): boolean {
    ...
    const correctedX = e.x - divPos.x + this.div.scrollLeft - this.screenOffset.x * this.zoom;
    const correctedY = e.y - divPos.y + this.div.scrollTop - this.screenOffset.y * this.zoom;
    const selectedClass = this.diagram.classViews.find(classView => classView
      .contains((correctedX - this.canvasCenter.x) / this.zoom + this.canvasCenter.x, (correctedY - this.canvasCenter.y) / this.zoom + this.canvasCenter.y));
  ...
  mousedown(e: MouseEvent): boolean {
    ...
    if (e.button === 0) {
      ...
      const x = (correctedX - this.canvasCenter.x) / this.zoom + this.canvasCenter.x;
      const y = (correctedY - this.canvasCenter.y) / this.zoom + this.canvasCenter.y;
      ...
      for (let i = this.diagram.classViews.length - 1; i >= 0; i--) {
        if (this.diagram.classViews[i].buildBottomRightCornerBox().contains(x, y)) {
          this.selectedClasses = [];
          this.selectedClassCorner = this.diagram.classViews[i];
          if (!this.isReadOnly) {
            // Bring the class view to front
            this.diagram.setClassViews(this.reorderDiagramDomain(this.selectedClassCorner, this.diagram));
          }
          break;
        }
      }
      ...
      if (!this.selectedClassCorner) {
        ...
        for (let i = this.diagram.classViews.length - 1; i >= 0; i--) {
          if (this.diagram.classViews[i].contains(x, y)) {
            if (this.selectedClasses.length === 0 || this.selectedClasses.indexOf(this.diagram.classViews[i]) === -1) {
      ...
      if (!this.selectedClassCorner && !this.selectedClasses.length) {
        for (const generalizationView of this.diagram.generalizationViews) {
          const val = generalizationView.findOrBuildPoint((correctedX - this.canvasCenter.x) / this.zoom + this.canvasCenter.x, (correctedY - this.canvasCenter.y) / this.zoom + this.canvasCenter.y, this.zoom, !this.isReadOnly);
          if (val) {
      ...
      if (!this.selectedClassCorner && !this.selectedClasses.length && !this.selectedPoint) {
        for (const associationView of this.diagram.associationViews) {
          const val = associationView.findOrBuildPoint((correctedX - this.canvasCenter.x) / this.zoom + this.canvasCenter.x, (correctedY - this.canvasCenter.y) / this.zoom + this.canvasCenter.y, this.zoom, !this.isReadOnly);
          if (val) {
            this.selectedPoint = val;
      ...
      if (!this.selectedClassCorner && !this.selectedClasses.length && !this.selectedPoint) {
        for (const propertyView of this.diagram.propertyViews) {
          const val = propertyView.findOrBuildPoint((correctedX - this.canvasCenter.x) / this.zoom + this.canvasCenter.x, (correctedY - this.canvasCenter.y) / this.zoom + this.canvasCenter.y, this.zoom, !this.isReadOnly);
          if (val) {
            this.selectedPoint = val;
  ...
  mousemove(e: MouseEvent): void {
    ...
    if (this.rightClick) {
      ...
      if (this.selectedClassCorner) {
        ...
        const correctedX = e.x - divPos.x + this.div.scrollLeft - this.screenOffset.x * this.zoom;
        const correctedY = e.y - divPos.y + this.div.scrollTop - this.screenOffset.y * this.zoom;
        const newMovingX = (correctedX - this.canvasCenter.x) / this.zoom + this.canvasCenter.x;
        const newMovingY = (correctedY - this.canvasCenter.y) / this.zoom + this.canvasCenter.y;
        // Make sure width and height are in range
        this.selectedClassCorner.setRectangle(new Rectangle(newMovingX - this.selectedClassCorner.position.x, newMovingY - this.selectedClassCorner.position.y));
      ...
      if (!this.selectionStart && this.selectedClasses.length) {
        if (!this.isReadOnly) {
          ...
          this.selectedClasses.forEach((selectedClass, idx) => {
            const selectedClassOldPos = this.selectedClassesOldPos.length > idx ? this.selectedClassesOldPos[idx] : undefined;
            if (selectedClassOldPos) {
              const newMovingX = correctedX / this.zoom - (this.clickX - selectedClassOldPos.oldPos.x);
              const newMovingY = correctedY / this.zoom - (this.clickY - selectedClassOldPos.oldPos.y);
      ...
      if (this.selectedPoint) {
        ...
        if (this.selectedPropertyOrAssociation) {
          this.selectedPropertyOrAssociation.changePoint(this.selectedPoint, updatedSelectedPoint);
        } else if (this.selectedInheritance) {
          this.selectedInheritance.changePoint(this.selectedPoint, updatedSelectedPoint);
        }
      ...
      if (this.selectionStart) {
        ...
        const startX = (this.selectionStart.x - this.canvasCenter.x + this.screenOffset.x) * this.zoom + this.canvasCenter.x;
        const startY = (this.selectionStart.y - this.canvasCenter.y + this.screenOffset.y) * this.zoom + this.canvasCenter.y;
        this.ctx.fillStyle = this.selectionBoxBorderColor;
        this.ctx.fillRect(startX, startY, correctedX - startX, correctedY - startY);
        this.ctx.strokeRect(startX, startY, correctedX - startX, correctedY - startY);
        this.selection = new PositionedRectangle(new Point(this.selectionStart.x, this.selectionStart.y), new Rectangle((correctedX - startX) / this.zoom, (correctedY - startY) / this.zoom));
        this.selectedClasses = [];
        for (const classView of this.diagram.classViews) {
          if (this.selection.boxContains(classView) || classView.boxContains(this.selection)) {
      ...
      const cX = (correctedX - this.canvasCenter.x) / this.zoom + this.canvasCenter.x;
      const cY = (correctedY - this.canvasCenter.y) / this.zoom + this.canvasCenter.y;
      this.mousedOverClassView = undefined;
      this.mousedOverProperty = undefined;
      for (const classView of this.diagram.classViews.slice().reverse()) {
        if (classView.contains(cX, cY)) {
          this.mousedOverClassView = classView;
          ...
          cursorY = cursorY + (classView.hideStereotypes ? 0 : _class.stereotypes.length * this.fontSize);
          cursorY = cursorY + (classView.hideTaggedValues ? 0 : _class.taggedValues.length * this.fontSize);
          ...
          for (const property of _class.getAllOwnedProperties()) {
            if (!this.hasPropertyView(classView, property)) {
              ...
              this.ctx.font = `${(this.fontSize - 1) * this.zoom}px ${this.fontFamily}`;
              const propertyName = this.propertyName(property);
              const txtMeasure = this.ctx.measureText(`${propertyName} : `).width;
              const typeMeasure = this.ctx.measureText(property.genericType.value.rawType.name).width;
              if (new PositionedRectangle(new Point(propX, propY), new Rectangle(txtMeasure + typeMeasure, this.fontSize * this.zoom)).contains(sX, sY)) {
                this.mousedOverProperty = property;
              }
