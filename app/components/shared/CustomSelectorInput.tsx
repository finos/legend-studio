...
const CustomMenuList: React.FC<{
  ...
  useEffect(() => { listRef.current?.scrollToItem(scrollToIndex) }, [scrollToIndex]);
  ...
  if (children.length) {
    return (
      ...
      >
        {({ index, style }): React.ReactElement<ListChildComponentProps> => <div style={style}>{children[index]}</div>}
      </FixedSizeList>
    );
  }
  // TODO: Maybe a no option thing here
  return (
    <div className={`selector-menu--no-match ${selectProps.darkMode ? 'selector-menu--dark' : ''}`}>
      No match found
    </div>
  );
};
...
const DropdownIndicator: React.FC = () =>
  <div className={`${STYLE_PREFIX}__indicator ${STYLE_PREFIX}__dropdown-indicator`}>
    <FaCaretDown />
  </div>;
...
const ClearIndicator: React.FC<{
  ...
  return (
    <div {...innerProps} ref={ref}>
      <div className={`${STYLE_PREFIX}__indicator ${STYLE_PREFIX}__clear-indicator`}>
        <FaTimes />
      </div>
    </div>
  );
...
export const CustomSelectorInput = forwardRef<SelectComponent, CustomSelectorInputProps>((props, ref) => {
  const { option, allowCreating, disabled, components, className, darkMode, hasError, ...innerProps } = props;
  ...
  const SelectComponent: React.ElementType = allowCreating ? CreatableSelect : Select;
  ...
  return (
    <SelectComponent
      ...
      className={`${stylePrefix} ${className} ${hasError ? 'selector-input--has-error' : ''}`}
