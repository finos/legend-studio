...
import { ParserError, ExecutionServerError } from 'EXEC/ExecutionServerError';
import { setErrorMarkers, disposeEditor, disableEditorHotKeys, baseTextEditorSettings } from 'Utilities/TextEditorUtil';
...
const LambdaErrorFeedback: React.FC<{
  ...
  return (
    <div className="lambda-editor__error-feedback">
      <div className="lambda-editor__error-feedback__error__message">{error.message}</div>
      {error instanceof ParserError &&
        <div className="lambda-editor__error-feedback__parsing-error__content">
          ...
          <div className="lambda-editor__error-feedback__parsing-error__message">fix error or discard changes to leave</div>
        </div>
      }
    </div>
  );
...
export const LambdaEditor = observer((props: {
  ...
  /**
   * WIP: when we pass in these expected types we should match a type as expected type if it's covariance, i.e. it is a subtype of
  ...
  const { className, lambdaEditorState, disabled, forceBackdrop, expectedType, onExpectedTypeLabelSelect, matchedExpectedType, forceExpansion } = props;
  const debouncedTransformStringToLambda = useMemo(() => disabled ? undefined : debounce(() => lambdaEditorState.convertLambdaGrammarStringToObject(), 1000), [lambdaEditorState, disabled]);
  ...
  return (
    <LambdaEditorInner
       ...
       *
       * So technically, we don't need to do the force-remount using `key`, but to be cleaner, we would prevent other
       * potential bugs, we do it anyway to reset everything.
       */
...
const LambdaEditorInner = observer((props: {
  className?: string;
  disabled: boolean;
  lambdaEditorState: LambdaEditorState;
  ...
  const { className, disabled, lambdaEditorState, transformStringToLambda, expectedType, onExpectedTypeLabelSelect, matchedExpectedType, forceBackdrop, forceExpansion } = props;
  ...
  const transformLambdaToString = (pretty: boolean): Promise<void> => {
    ...
    return lambdaEditorState.convertLambdaObjectToGrammarString(pretty ? RenderStyle.PRETTY : RenderStyle.STANDARD).catch(applicationStore.alertIllegalUnhandledError);
  };
  ...
  const toggleExpandedMode = (): void => {
    if (!forceExpansion && !parserError) {
      transformLambdaToString(!isExpanded).catch(applicationStore.alertIllegalUnhandledError);
  ...
  if (editor) {
    ...
    onDidChangeModelContentEventDisposer.current = editor.onDidChangeModelContent(() => {
       ...
       * This stub lambda sometimes does not even get registered in change detection and will cause the user to lose data
       */
      transformStringToLambda?.cancel();
      transformStringToLambda?.()?.catch(applicationStore.alertIllegalUnhandledError);
    });
    ...
    onKeyDownEventDisposer.current = editor.onKeyDown(event => {
      if (event.keyCode === KeyCode.F8) {
        ...
        transformStringToLambda?.cancel();
        editorStore.graphState.checkLambdaParsingError(lambdaEditorState, !disabled, () => editorStore.toggleTextMode()).catch(applicationStore.alertIllegalUnhandledError);
      } else if (event.keyCode === KeyCode.F9) {
        event.preventDefault();
        event.stopPropagation();
        transformStringToLambda?.cancel();
        editorStore.graphState.checkLambdaParsingError(lambdaEditorState, !disabled, () => editorStore.graphState.globalCompileInFormMode()).catch(applicationStore.alertIllegalUnhandledError);
      } else if (event.keyCode === KeyCode.F10) {
        event.preventDefault();
        event.stopPropagation();
        transformStringToLambda?.cancel();
        editorStore.graphState.checkLambdaParsingError(lambdaEditorState, !disabled, () => editorStore.graphState.graphGenerationState.globalGenerate()).catch(applicationStore.alertIllegalUnhandledError);
      }
    });
    ...
    if (currentValue !== value) { editor.setValue(value) }
  ...
  return (
    <>
      <div className={clsx('lambda-editor', className, { 'lambda-editor__expanded': isExpanded })}>
        <ReactResizeDetector
          handleWidth={true}
          handleHeight={true}
          onResize={handleResize}
        >
          <div data-testid={TEST_ID.LAMBDA_EDITOR__EDITOR_INPUT} className="lambda-editor__editor__input">
        ...
        {Boolean(expectedType) &&
          <div className="lambda-editor__editor__info">
            {onExpectedTypeLabelSelect &&
              ...
              >{expectedType?.name ?? 'unknown'}</button>
            }
            {!onExpectedTypeLabelSelect &&
              <div className={clsx('lambda-editor__editor__expected-return-type',
                { 'lambda-editor__editor__expected-return-type--highlighted': matchedExpectedType?.() }
              )}>{expectedType?.name ?? 'unknown'}</div>
            }
          </div>
        }
        {!forceExpansion &&
      ...
      <LambdaErrorFeedback error={parserError ?? compilationError} discardChanges={discardChanges} />
    </>
  );
});
