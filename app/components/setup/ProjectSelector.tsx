...
const formatOptionLabel = (option: ProjectSelectOption): React.ReactNode => (
  <div className="setup__project__label">
    <div className={clsx([`setup__project__label__tag setup__project__label__tag--${option.tag.toLowerCase()}`, { 'setup__project__label__tag--disabled': option.disabled }])}>{option.tag}</div>
    <div className="setup__project__label__name">{option.label}</div>
  </div>
);

export const ProjectSelector = observer((props: {
  ...
  const isLoadingOptions = setupStore.loadProjectsState === ACTION_STATE.IN_PROGRESS;
  ...
  const onSelectionChange = (val: ProjectSelectOption | null): void => {
    if ((val !== null || selectedOption !== null) && (!val || !selectedOption || val.value !== selectedOption.value)) {
      ...
      if (val && !setupStore.currentProjectWorkspaces) {
        setupStore.fetchWorkspaces(val.value).catch(applicationStore.alertIllegalUnhandledError);
      }
  ...
  return (
    <div className="setup-selector">
      ...
      <CustomSelectorInput
        ...
        isOptionDisabled={(option: { disabled: boolean }): boolean => option.disabled}
      />
    </div>
  );
}, { forwardRef: true });
