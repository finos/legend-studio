...
import { WorkspaceSelectOption } from 'SDLC/workspace/Workspace';
import { FaPlus } from 'react-icons/fa';
import { getSetupRoute } from 'Stores/RouterConfig';
import { ACTION_STATE } from 'Const';
import { useApplicationStore } from 'Stores/ApplicationStore';

export const WorkspaceSelector = observer((props: {
  ...
  const selectedOption = options.find(option => option.value === currentWorkspaceId) ?? null;
  const isLoadingOptions = setupStore.loadProjectsState === ACTION_STATE.IN_PROGRESS || setupStore.loadWorkspacesState === ACTION_STATE.IN_PROGRESS;
  ...
  const onSelectionChange = (val: WorkspaceSelectOption | null): void => {
    if ((val !== null || selectedOption !== null) && (!val || !selectedOption || val.value !== selectedOption.value)) {
      ...
      applicationStore.historyApiClient.push(getSetupRoute(setupStore.currentProjectId ?? '', val?.value));
    }
  };
  ...
  }, [setupStore.currentProjectWorkspaces, setupStore.currentProjectId, currentWorkspaceId, onChange]);
