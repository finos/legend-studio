...
import { InferableValue } from 'MM/model/InferableValue';

export abstract class InferableMappingElementIdValue extends InferableValue<string, string | undefined> {
  protected defaultValue!: string;

  @computed get isDefault(): boolean { return this.defaultValue === this.value }
}

export class InferableMappingElementIdExplicitValue extends InferableMappingElementIdValue {
  static create(value: string, targetPath: string): InferableMappingElementIdExplicitValue {
    const id = new InferableMappingElementIdExplicitValue();
    id.value = value;
    id.defaultValue = fromElementPathToMappingElementId(targetPath);
    return id;
  }
...
export class InferableMappingElementIdImplicitValue extends InferableMappingElementIdValue {
  ...
  static create(value: string, targetPath: string, input: string | undefined): InferableMappingElementIdImplicitValue {
    const id = new InferableMappingElementIdImplicitValue();
    id.value = value;
    id.defaultValue = fromElementPathToMappingElementId(targetPath);
    id.input = input;
    return id;
